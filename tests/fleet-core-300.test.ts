...
describe("Fleet Core 3.0", () => {
  ...
  it("używa dedykowanego, zoptymalizowanego loadera i workspace zamiast starego shell", () => {
  ...
  it("udostępnia wszystkie dziewięć sekcji operacyjnych Fleet Core 3.0", () => {
    ...
    for (const tab of ["dashboard", "vehicles", "waiting", "operations", "service", "documents", "equipment", "damages", "costs"]) {
      expect(workspace).toContain(`id: "${tab}"`);
    }
    for (const label of ["Pulpit", "Pojazdy", "Poczekalnia AI", "Eksploatacja", "Serwis", "Dokumenty i terminy", "Wyposażenie i opony", "Szkody i bezpieczeństwo", "Koszty i wykorzystanie"]) {
  ...
  it("obsługuje komplet operacji, integracje między modułami i kontrolę zatwierdzania", () => {
    ...
    for (const action of [
      "vehicle_create", "vehicle_update", "meter_reading", "fuel_entry", "trip_create", "service_create", "service_close",
      "service_plan_create", "service_item_create", "document_create", "component_create", "component_remove", "damage_create",
      "damage_update", "allocation_create", "cost_rate_create", "qualification_requirement_create", "vehicle_check_create",
      "asset_assign", "asset_unassign", "anomaly_resolve", "ai_review_accept", "ai_review_ignore", "ai_undo"
