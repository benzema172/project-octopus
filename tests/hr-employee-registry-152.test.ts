...
describe("Kadry 1.5.2 employee registry actions", () => {
  ...
  it("supports editing, archiving, restoring and safe permanent deletion", () => {
    expect(registry).toContain('action: "update"');
    expect(registry).toContain('changeEmployeeState(String(editEmployee.id), "archive")');
    expect(registry).toContain('changeEmployeeState(String(editEmployee.id), "delete")');
    expect(managementRoute).toContain('type Action = "update" | "archive" | "restore" | "delete"');
