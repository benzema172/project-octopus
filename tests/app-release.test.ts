...
describe("application release badge", () => {
  it("publishes the official Project Octopus 1.4.12 release and date", () => {
    const pkg = JSON.parse(readFileSync("package.json", "utf8")) as { version: string };
    expect(pkg.version).toBe("1.4.12");
    expect(APP_RELEASE.version).toBe("1.4.12");
    expect(APP_RELEASE.introducedAt).toBe("28.08.2026");
    expect(APP_RELEASE_LABEL).toContain("v1.4.12");
