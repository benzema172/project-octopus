...
describe("application release badge", () => {
  it("publishes the official Project Octopus 1.3.3 release and date", () => {
    const pkg = JSON.parse(readFileSync("package.json", "utf8")) as { version: string };
    expect(pkg.version).toBe("1.3.3");
    expect(APP_RELEASE.version).toBe("1.3.3");
    expect(APP_RELEASE.introducedAt).toBe("26.08.2026");
    expect(APP_RELEASE_LABEL).toContain("v1.3.3");
