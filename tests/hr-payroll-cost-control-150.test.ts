...
describe("Kadry Core 3.0 — wynagrodzenia i pełny koszt pracodawcy", () => {
  it("calculates the employer total and effective hourly cost from the detailed breakdown", () => {
    const result = calculateCompensation({ netMonthlyPay: 6_200, grossMonthlyPay: 8_600, employerContributions: 1_760, otherMonthlyCosts: 300, nominalMonthlyHours: 168 });
  ...
  it("preserves legacy monthly and hourly costs when no breakdown exists", () => {
    expect(calculateCompensation({ legacyMonthlyCost: 10_000, legacyHourlyCost: 62.5 })).toMatchObject({ totalEmployerCost: 10_000, effectiveHourlyCost: 62.5, hasDetailedBreakdown: false });
  });
  ...
  it("connects current employee forms, profile, API and export to payroll cost data", () => {
    const create = read("components/company/hr/hr-employee-create-300.tsx");
    ...
    expect(registry).toContain("HrCompensationFields150");
