...
describe("Project Octopus HR hybrid settlement model", () => {
  it("uses the operational hourly rate as the investment labor cost", () => {
    const result = calculateLaborControl({
      ...
      employments: [{
        ...
        settlement_model: "hourly_with_monthly_base",
        operational_net_hourly_rate: 30,
        hourly_cost: 30,
        monthly_cost: 6200,
        net_monthly_pay: 4000,
        gross_monthly_pay: 5100,
        nominal_monthly_hours: 160
      }],
  ...
  it("adds the model selector and stores it after employee creation", () => {
    const fields = readFileSync("components/company/hr/hr-compensation-fields-150.tsx", "utf8");
    const create = readFileSync("components/company/hr/hr-employee-create-153.tsx", "utf8");
    ...
    expect(create).toContain("/api/company/hr/employee-compensation");
    expect(create).toContain("formalną podstawę brutto miesięczną");
