...
as $$
declare v_order_id uuid;v_line_id uuid;v_wbs_id uuid;v_total numeric;
begin
  ...
  if p_source_request_id is not null then perform 1 from public.material_requests where id=p_source_request_id and project_id=p_project_id; if not found then raise exception 'Wniosek materiałowy nie należy do inwestycji.'; end if; end if;
  ...
  values(p_workspace_id,p_project_id,p_counterparty_id,p_source_request_id,trim(p_order_number),'ordered',coalesce(p_ordered_at,current_date),p_expected_at,'PLN',v_total,p_actor_id) returning id into v_order_id;
...
insert into public.app_schema_versions(version) values ('20260817_094_purchase_order_workflow') on conflict(version) do update set applied_at=excluded.applied_at;
commit;
