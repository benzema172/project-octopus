...
-- Fleet Core 3.0 — cyfrowy paszport, eksploatacja, TCO i bezpieczne AI.
...
create index if not exists vehicle_service_plans_vehicle_idx on public.vehicle_service_plans(workspace_id,vehicle_id,active,next_due_date);
...
create index if not exists vehicle_service_items_order_idx on public.vehicle_service_items(workspace_id,service_order_id);
...
create index if not exists vehicle_components_vehicle_idx on public.vehicle_components(workspace_id,vehicle_id,active,component_type);
...
create index if not exists fleet_ai_decision_events_review_idx on public.fleet_ai_decision_events(workspace_id,review_id,created_at desc);
...
create index if not exists fleet_cost_links_vehicle_idx on public.fleet_cost_links(workspace_id,vehicle_id,occurred_at desc);
...
-- RLS: klient widzi i modyfikuje tylko dane domeny Fleet aktywnej firmy.
do $$
declare t text;
begin
  foreach t in array array[
    'vehicle_service_plans','vehicle_service_items','vehicle_components','fleet_document_reviews',
    'fleet_ai_feedback','fleet_ai_decision_events','fleet_anomalies','fleet_cost_links'
  ] loop
...
-- Dokumenty z Wrzutni Floty stają się kontrolowanymi propozycjami AI.
...
begin
  ...
  v_reg:=nullif(coalesce(v_business->>'registrationNumber',new.payload->>'registrationNumber',v_business #>> '{vehicle,registrationNumber}'),'');
  v_vin:=nullif(coalesce(v_business->>'vin',new.payload->>'vin',v_business #>> '{vehicle,vin}'),'');
  ...
  v_issue:=nullif(coalesce(v_business->>'issueDate',new.payload->>'issueDate'),'');
  v_valid_from:=nullif(coalesce(v_business->>'validFrom',new.payload->>'validFrom'),'');
  v_valid_until:=nullif(coalesce(v_business->>'validUntil',new.payload->>'validUntil'),'');
  ...
  insert into public.fleet_document_reviews(
    ...
    private.try_numeric(coalesce(v_business->>'mileage',new.payload->>'mileage')),
    private.try_numeric(coalesce(v_business->>'engineHours',new.payload->>'engineHours')),
    private.try_numeric(coalesce(v_business->>'fuelLiters',new.payload->>'fuelLiters')),
    nullif(coalesce(v_business->>'fuelType',new.payload->>'fuelType'),'') ,
    nullif(coalesce(v_business->>'serviceType',new.payload->>'serviceType'),'') ,
    nullif(coalesce(v_business->>'workshopName',new.payload->>'workshopName'),'') ,
    nullif(coalesce(v_business->>'claimNumber',new.payload->>'claimNumber'),'') ,
...
-- Odczyt licznika: spadek nigdy nie aktualizuje źródła prawdy i tworzy alert zamiast cichej korekty.
...
begin
  ...
  if v_verified then
    update public.vehicles set current_mileage=coalesce(greatest(current_mileage,p_mileage),p_mileage,current_mileage),current_engine_hours=coalesce(greatest(current_engine_hours,p_engine_hours),p_engine_hours,current_engine_hours),updated_at=now() where id=p_vehicle_id;
  else
...
begin
  ...
  if r.status='applied' and lower(p_action)<>'ignore' then raise exception 'Dokument został już zastosowany.'; end if;
  if lower(p_action)='ignore' then
  ...
  if v_type like '%fuel%' or v_type like '%paliw%' or r.fuel_liters is not null then
    ...
    values(p_workspace_id,p_vehicle_id,r.project_id,coalesce(r.document_date,current_date)::timestamptz,coalesce(r.fuel_liters,0),coalesce(r.amount,0),r.mileage,r.fuel_type,r.document_id)
...
-- Lekki snapshot KPI: bez szerokiego fan-outu po stronie klienta.
...
grant execute on function public.undo_fleet_ai_decision_300(uuid,uuid,uuid,uuid) to service_role;
