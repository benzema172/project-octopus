...
-- Fleet Intelligence 4.0 — readiness, Mission Fit, predykcja, AI Controller, walkaround, gwarancje, warsztaty,
-- service kits, decyzje majątkowe, teczki zdarzeń, driver score i transformacja EV.
...
begin
  for v in select * from public.vehicles where workspace_id=p_workspace_id and status<>'sold' loop
    ...
    from public.vehicle_required_qualifications rq
    where rq.workspace_id=p_workspace_id and rq.vehicle_id=v.id and not exists(
      select 1 from public.qualifications q where q.workspace_id=p_workspace_id and q.employee_id=v.responsible_employee_id
      and lower(q.qualification_type)=lower(rq.qualification_type) and q.status not in('expired','revoked') and (q.valid_until is null or q.valid_until>=current_date)
    );
...
begin
  ...
  for v in select * from public.vehicles where workspace_id=p_workspace_id and status='active' loop
    ...
    v_total:=round((coalesce(v.readiness_score,70)*0.35)+(v_cap*0.20)+(v_qual*0.15)+(v_asset*0.10)+(v_prox*0.10)+(v_avail*0.10),2);
...
begin
  ...
  for r in
    with x as (
      select f.*,lag(mileage) over(partition by vehicle_id order by fueled_at) prev_mileage
      from public.fuel_entries f where workspace_id=p_workspace_id and fueled_at>=now()-interval '180 days' and mileage is not null
    ),c as (select *,case when mileage>prev_mileage then liters/(mileage-prev_mileage)*100 end consumption from x)
    select * from c where consumption is not null and consumption>0
  loop
...
begin
  ...
  select d.workspace_id,d.vehicle_id,coalesce(d.system,'diagnostyka'),'dtc_failure_risk',case when d.severity='critical' then .92 else .68 end,case when d.severity='critical' then 7 else 30 end,null,null,current_date+case when d.severity='critical' then 7 else 30 end,
    jsonb_build_object('dtc',d.code,'description',d.description,'capturedAt',d.captured_at),'rules_v1'
  ...
  insert into public.fleet_maintenance_predictions(workspace_id,vehicle_id,system_name,prediction_type,risk_probability,horizon_days,horizon_km,horizon_engine_hours,predicted_date,evidence,model)
  select p.workspace_id,p.vehicle_id,p.service_type,'service_due_risk',
    case when (p.next_due_date is not null and p.next_due_date<=current_date) or (p.next_due_mileage is not null and coalesce(v.current_mileage,0)>=p.next_due_mileage) or (p.next_due_engine_hours is not null and coalesce(v.current_engine_hours,0)>=p.next_due_engine_hours) then .9 else .62 end,
...
begin
  ...
  where e.workspace_id=p_workspace_id and e.status='active'
  group by e.id
...
begin
  ...
  with base as (
    select s.*,avg(s.cost) over(partition by s.service_type) type_avg,
      extract(epoch from(coalesce(s.closed_at,current_date)-s.opened_at))/86400 days,
      lag(s.closed_at) over(partition by s.vehicle_id,s.service_type order by s.opened_at) prev_closed
    ...
    select workshop_counterparty_id,count(*) samples,avg(cost) avg_cost,avg(days) avg_days,
      count(*) filter(where prev_closed is not null and opened_at-prev_closed<=45) repeats,
      ...
      avg(greatest(0,100-greatest(0,days-3)*4)) time_score
    from base group by workshop_counterparty_id
  )
  select p_workspace_id,workshop_counterparty_id,
    round((coalesce(price_score,100)*.4+coalesce(time_score,100)*.3+greatest(0,100-(repeats*20))* .3),1),
    round(coalesce(price_score,100),1),round(coalesce(time_score,100),1),greatest(0,100-(repeats*20)),samples,round(avg_cost,2),round(avg_days,1),repeats,
    jsonb_build_object('samples',samples,'averageCost',avg_cost,'averageDays',avg_days,'repeatRepairs',repeats),now()
  from agg
...
begin
  ...
  select s.workspace_id,s.vehicle_id,s.id,coalesce(s.source_document_id,s.document_id),'vehicle',coalesce(s.cost,0),
    case when v.warranty_until is not null and v.warranty_until>=coalesce(s.closed_at,s.opened_at) and v.warranty_mileage_limit is not null and coalesce(s.current_mileage,v.current_mileage,0)<=v.warranty_mileage_limit then .9 else .75 end,
    concat_ws(' ',case when v.warranty_until>=coalesce(s.closed_at,s.opened_at) then 'Naprawa wykonana w okresie gwarancji.' end,case when v.warranty_mileage_limit is not null and coalesce(s.current_mileage,v.current_mileage,0)<=v.warranty_mileage_limit then 'Przebieg mieści się w limicie gwarancyjnym.' end),
    'candidate',coalesce(v.warranty_until,current_date+30)
  from public.service_orders s join public.vehicles v on v.id=s.vehicle_id
  where s.workspace_id=p_workspace_id and s.status='closed' and coalesce(s.cost,0)>0
    and ((v.warranty_until is not null and v.warranty_until>=coalesce(s.closed_at,s.opened_at)) or (v.warranty_mileage_limit is not null and coalesce(s.current_mileage,v.current_mileage,0)<=v.warranty_mileage_limit))
...
begin
  ...
  with u as (
    select v.id vehicle_id,
      least(100,coalesce(sum(g.duration_minutes),0)/(90*8*60)*100) utilization,
      ...
      coalesce((select count(*) from public.fleet_missions m where m.workspace_id=p_workspace_id and m.selected_vehicle_id=v.id and m.planned_start between now() and now()+interval '90 days' and m.status in('planned','assigned')),0) future_missions,
      v.production_year
    from public.vehicles v left join public.fleet_geofence_visits g on g.vehicle_id=v.id and g.entered_at>=now()-interval '90 days'
    where v.workspace_id=p_workspace_id and v.status<>'sold' group by v.id,v.production_year
  )
  select p_workspace_id,vehicle_id,
    case when utilization<18 and monthly_tco>0 then 'sell' when utilization<30 and monthly_tco>0 then 'rent_out' when production_year is not null and production_year<=extract(year from current_date)::int-9 and (maint/nullif(total,0))>.35 then 'replace' else 'keep' end,
    round(case when utilization<18 and monthly_tco>0 then 85 when utilization<30 and monthly_tco>0 then 70 when production_year is not null and production_year<=extract(year from current_date)::int-9 and (maint/nullif(total,0))>.35 then 80 else 55 end,1),
    ...
    case when utilization<18 and monthly_tco>0 then 'Bardzo niskie wykorzystanie przy realnym koszcie utrzymania.' when utilization<30 and monthly_tco>0 then 'Niskie wykorzystanie — warto porównać utrzymanie z wynajmem lub udostępnieniem.' when production_year is not null and production_year<=extract(year from current_date)::int-9 and (maint/nullif(total,0))>.35 then 'Wysoki udział napraw i wiek pojazdu uzasadniają analizę wymiany.' else 'Brak silnego sygnału do zmiany sposobu finansowania lub posiadania.' end,
    jsonb_build_object('futureMissions90d',future_missions,'maintenanceCost90d',maint,'totalCost90d',total),'new',now()
  from u
...
begin
  ...
  with daily as (
    select vehicle_id,started_at::date d,sum(distance_km) km from public.trips where workspace_id=p_workspace_id and started_at>=now()-interval '120 days' group by vehicle_id,started_at::date
  ),agg as (select vehicle_id,avg(km) avg_km,max(km) max_km from daily group by vehicle_id),base as (
    select v.id,coalesce(a.avg_km,0) avg_km,coalesce(a.max_km,0) max_km,
      case when coalesce(a.max_km,0)<=180 then 100 when coalesce(a.max_km,0)<=260 then 75 when coalesce(a.max_km,0)<=350 then 45 else 20 end route_fit,
      case when exists(select 1 from public.fleet_geofences g where g.workspace_id=p_workspace_id and g.geofence_type='base' and g.active=true) then 80 else 45 end charge_fit
    from public.vehicles v left join agg a on a.vehicle_id=v.id where v.workspace_id=p_workspace_id and v.status<>'sold' and coalesce(v.drive_type,'ice') not in('ev')
  )
  select p_workspace_id,id,round(route_fit*.7+charge_fit*.3,1),round(avg_km,1),round(max_km,1),null,route_fit,charge_fit,
    case when route_fit*.7+charge_fit*.3>=75 then 'Silny kandydat do analizy EV.' when route_fit*.7+charge_fit*.3>=55 then 'Kandydat warunkowy — sprawdź ładowanie i sezonowy zasięg.' else 'Na obecnym profilu tras EV wymaga dokładniejszej analizy.' end,
    jsonb_build_object('note','Ocena trasy bez założenia cen energii i paliwa; koszty należy policzyć z realnych taryf firmy.'),now() from base
...
with balance as (
  ...
  where m.workspace_id=p_workspace_id and m.status='approved' group by l.stock_item_id
),x as (
  ...
  from public.fleet_service_kits k join public.fleet_service_kit_items i on i.kit_id=k.id left join balance b on b.stock_item_id=i.stock_item_id
  where k.workspace_id=p_workspace_id and k.active=true
)
...
begin
  ...
  insert into public.fleet_incident_evidence(workspace_id,vault_id,evidence_type,source_table,source_id,description,captured_at)
  values(p_workspace_id,v_id,'damage_case','damage_cases',d.id,'Rekord szkody',d.occurred_at);
  ...
  select p_workspace_id,v_id,'vehicle_document',vd.document_id,'vehicle_documents',vd.id,vd.document_type,coalesce(vd.created_at,d.occurred_at)
  from public.vehicle_documents vd where vd.workspace_id=p_workspace_id and vd.vehicle_id=d.vehicle_id and vd.document_id is not null and (vd.valid_until is null or vd.valid_until>=d.occurred_at::date);
...
begin
  ...
  from public.vehicles v join lateral(select * from public.fleet_readiness_snapshots s where s.vehicle_id=v.id order by s.calculated_at desc limit 1)s on true
  where v.workspace_id=p_workspace_id and s.status<>'ready'
  ...
  from public.fleet_maintenance_predictions p where p.workspace_id=p_workspace_id and p.status='open' and p.risk_probability>=.62
  ...
  insert into public.fleet_ai_recommendations(workspace_id,vehicle_id,recommendation_type,dedupe_key,title,description,severity,confidence,recommended_action,action_payload,generated_by,status,valid_until,updated_at)
  select p_workspace_id,a.vehicle_id,'fuel_anomaly','fuel:'||a.id,a.title,a.description,a.severity,.9,'review_fuel',jsonb_build_object('anomalyId',a.id),'rules_v1','new',now()+interval '14 days',now()
  from public.fleet_anomalies a where a.workspace_id=p_workspace_id and a.status='open' and a.anomaly_type like 'fuel_%'
  ...
  from public.fleet_warranty_claims w where w.workspace_id=p_workspace_id and w.status='candidate'
  ...
  insert into public.fleet_ai_recommendations(workspace_id,vehicle_id,recommendation_type,dedupe_key,title,description,severity,confidence,recommended_action,action_payload,generated_by,status,valid_until,updated_at)
  select p_workspace_id,d.vehicle_id,'asset_strategy','asset:'||d.vehicle_id,'Strategia pojazdu: '||d.recommendation,d.reason,case when d.recommendation in('sell','replace') then 'warning' else 'info' end,d.score/100,'open_asset_decision',jsonb_build_object('decisionId',d.id),'rules_v1','new',now()+interval '30 days',now()
  from public.fleet_asset_decisions d where d.workspace_id=p_workspace_id and d.recommendation<>'keep' and d.status='new'
...
-- Automatyczna interpretacja materiału walkaround wrzuconego przez Wrzutnię Floty.
...
begin
  ...
  where not exists(select 1 from public.fleet_walkaround_inspections where workspace_id=new.workspace_id and document_id=new.document_id)
...
-- Funkcje inteligencji są wykonywane przez warstwę serwerową; klient dostaje tylko bezpieczne odczyty i dedykowane akcje API.
do $$
