...
create index if not exists processing_jobs_attention_idx on public.processing_jobs(workspace_id,status,available_at,created_at desc);
create index if not exists process_deviations_attention_idx on public.process_deviations(workspace_id,status,severity,created_at desc);
create index if not exists commitments_attention_idx on public.commitments(workspace_id,status,expected_date);
create index if not exists project_anomalies_attention_idx on public.project_anomalies(workspace_id,status,severity,detected_at desc);
create index if not exists closeout_requirements_attention_idx on public.closeout_requirements(workspace_id,status,due_at);

create or replace function public.get_company_action_center(p_workspace_id uuid,p_limit integer default 80)
returns table(
  item_key text, domain text, severity text, priority integer, title text, detail text,
  project_id uuid, entity_type text, entity_id text, href text,
  due_at timestamptz, amount numeric, created_at timestamptz
)
language sql
stable
security definer
...
as $$
with allocation_by_line as (
  select fa.source_line_id,
         sum(fa.amount) filter(where coalesce(fa.status,'approved') not in ('rejected','cancelled','void')) allocated
  from public.financial_allocations fa
  where fa.workspace_id=p_workspace_id and fa.source_type='invoice' and fa.source_line_id is not null
  group by fa.source_line_id
), invoice_allocation as (
  select i.id,i.invoice_number,i.due_date,
         sum(coalesce(il.net_amount,0)) line_net,
         sum(coalesce(a.allocated,0)) allocated_net
  ...
  group by i.id,i.invoice_number,i.due_date
), stock_balance as (
  select stock_item_id,sum(quantity)::numeric quantity
  from public.get_stock_balances(p_workspace_id)
  group by stock_item_id
), items(item_key,domain,severity,priority,title,detail,project_id,entity_type,entity_id,href,due_at,amount,created_at) as (
  select
    'job:'||j.id::text,
    'documents'::text,
    case when j.status='dead_letter' then 'critical' else 'warning' end,
    case when j.status='dead_letter' then 100 else 90 end,
    case when j.status='dead_letter' then 'Dokument trafił do dead-letter' else 'Przetwarzanie dokumentu nie powiodło się' end,
    concat_ws(' · ',j.stage,nullif(j.error_code,''),nullif(j.error_message,'')),
    j.project_id,'processing_job'::text,j.id::text,
    case when j.project_id is not null then '/workspace/projects/'||j.project_id::text||'/documentation' else '/workspace/companies/'||p_workspace_id::text||'/documents' end,
    j.available_at,j.estimated_cost,j.created_at
  from public.processing_jobs j
  where j.workspace_id=p_workspace_id and j.status in ('failed','error','dead_letter')

  union all
  select
    'inbox:'||bi.id::text,
    case when bi.document_type in ('invoice','purchase_invoice','sales_invoice') then 'finance' else 'documents' end,
    'warning',88,'Business Inbox wymaga interwencji',
    concat_ws(' · ',bi.source_channel,bi.document_type,nullif(bi.processing_error,'')),
    bi.project_id,'business_inbox_item',bi.id::text,
    ...
    bi.received_at,null::numeric,bi.received_at
  from public.business_inbox_items bi
  where bi.workspace_id=p_workspace_id and (bi.status in ('error','failed','review') or bi.processing_error is not null)

  union all
  select
    'invoice-allocation:'||ia.id::text,'finance',
    ...
    null::uuid,'invoice',ia.id::text,'/workspace/companies/'||p_workspace_id::text||'/finances',
    ia.due_date::timestamptz,greatest(coalesce(ia.line_net,0)-coalesce(ia.allocated_net,0),0),now()
  from invoice_allocation ia
  where coalesce(ia.line_net,0)-coalesce(ia.allocated_net,0)>0.01

  union all
  select
    'invoice-overdue:'||i.id::text,'finance',
    ...
    null::uuid,'invoice',i.id::text,'/workspace/companies/'||p_workspace_id::text||'/finances',
    i.due_date::timestamptz,greatest(coalesce(i.gross_amount,0)-coalesce(i.paid_amount,0),0),i.created_at
  from public.invoices i
  where i.workspace_id=p_workspace_id and i.direction='purchase' and i.due_date<current_date
    and coalesce(i.paid_amount,0)+0.01<coalesce(i.gross_amount,0)
    and coalesce(i.status,'') not in ('cancelled','void')

  union all
  select
    'commitment:'||c.id::text,'finance',
    ...
    c.project_id,'commitment',c.id::text,'/workspace/companies/'||p_workspace_id::text||'/finances',
    c.expected_date::timestamptz,c.amount,c.created_at
  from public.commitments c
  where c.workspace_id=p_workspace_id and c.status in ('open','approved') and c.expected_date<=current_date+14

  union all
  select
    'match:'||pm.id::text,'finance',
    ...
    89,'3-way match wymaga decyzji',
    'Różnica ilości '||coalesce(pm.quantity_variance,0)::text||', cena '||coalesce(round(pm.price_variance_percent,2),0)::text||'%',
    pm.project_id,'procurement_match',pm.id::text,'/workspace/companies/'||p_workspace_id::text||'/finances',
    null::timestamptz,null::numeric,pm.updated_at
  ...
  select
    ...
    d.title,d.detail,d.project_id,'process_deviation',d.id::text,
  ...
  select
    ...
    null::uuid,'stock_item',si.id::text,'/workspace/companies/'||p_workspace_id::text||'/warehouse',
    null::timestamptz,null::numeric,si.created_at
  ...
  union all
  select
    'inventory-count:'||ic.id::text,'warehouse','info',52,
    'Inwentaryzacja pozostaje otwarta','Spis z '||ic.count_date::text||' ma status '||ic.status,
    null::uuid,'inventory_count',ic.id::text,'/workspace/companies/'||p_workspace_id::text||'/warehouse',
    (ic.count_date+7)::timestamptz,null::numeric,ic.created_at
  ...
  select
    ...
    null::uuid,'medical_exam',me.id::text,'/workspace/companies/'||p_workspace_id::text||'/hr',
    me.valid_until::timestamptz,null::numeric,me.created_at
  ...
  select
    ...
    null::uuid,'qualification',q.id::text,'/workspace/companies/'||p_workspace_id::text||'/hr',
    q.valid_until::timestamptz,null::numeric,q.created_at
  ...
  union all
  select
    'leave:'||lr.id::text,'hr','info',57,'Wniosek urlopowy czeka na decyzję',
    e.first_name||' '||e.last_name||' · '||lr.date_from::text||'–'||lr.date_to::text,
    null::uuid,'leave_request',lr.id::text,'/workspace/companies/'||p_workspace_id::text||'/hr',
    lr.date_from::timestamptz,null::numeric,lr.created_at
  ...
  union all
  select
    'timesheet:'||t.id::text,'hr','warning',64,'Karta czasu wymaga zatwierdzenia',
    e.first_name||' '||e.last_name||' · '||t.work_date::text||' · '||coalesce(t.hours,0)::text||' h',
    t.project_id,'timesheet',t.id::text,'/workspace/companies/'||p_workspace_id::text||'/hr',
    (t.work_date+3)::timestamptz,null::numeric,t.created_at
  ...
  select
    ...
    null::uuid,'vehicle_document',vd.id::text,'/workspace/companies/'||p_workspace_id::text||'/fleet',
    vd.valid_until::timestamptz,null::numeric,vd.created_at
  ...
  select
    ...
    null::uuid,'service_order',so.id::text,'/workspace/companies/'||p_workspace_id::text||'/fleet',
    so.next_due_date::timestamptz,so.cost,so.created_at
  ...
  union all
  select
    'damage:'||dc.id::text,'fleet','warning',68,'Otwarta szkoda pojazdu',
    coalesce(v.registration_number,v.make||' '||v.model)||' · '||left(coalesce(dc.description,''),160),
    null::uuid,'damage_case',dc.id::text,'/workspace/companies/'||p_workspace_id::text||'/fleet',
    null::timestamptz,dc.cost,dc.created_at
  ...
  union all
  select
    'anomaly:'||pa.id::text,'investments',pa.severity,
    case pa.severity when 'critical' then 99 when 'high' then 90 when 'warning' then 78 else 65 end,
    pa.title,pa.detail,pa.project_id,'project_anomaly',pa.id::text,'/workspace/projects/'||pa.project_id::text,
  ...
  union all
  select
    'schedule:'||si.id::text,'investments','warning',76,'Pozycja harmonogramu po terminie',
    p.name||' · '||si.title||' · postęp '||coalesce(si.progress_percent,0)::text||'%',
    si.project_id,'schedule_item',si.id::text,'/workspace/projects/'||si.project_id::text||'/schedule',
    si.end_date::timestamptz,coalesce(si.planned_value,0)-coalesce(si.actual_value,0),si.end_date::timestamptz
  ...
  union all
  select
    'closeout:'||cr.id::text,'investments',
    case when cr.due_at<now() then 'critical' else 'warning' end,
    case when cr.due_at<now() then 93 else 73 end,
    'Wymaganie odbiorowe nie jest gotowe',p.name||' · '||cr.title,
    cr.project_id,'closeout_requirement',cr.id::text,'/workspace/projects/'||cr.project_id::text||'/closeout',
    cr.due_at,null::numeric,cr.created_at
  from public.closeout_requirements cr join public.projects p on p.id=cr.project_id
  where cr.workspace_id=p_workspace_id and cr.required=true
    and cr.status not in ('done','accepted','approved','completed') and cr.due_at<=now()+interval '30 days'

  union all
  select
    'project-end:'||p.id::text,'investments',
    ...
    p.name||' · '||p.contract_end::text,p.id,'project',p.id::text,'/workspace/projects/'||p.id::text,
    p.contract_end::timestamptz,p.contract_value,p.updated_at
    ...
    where n.workspace_id=p_workspace_id and n.read_at is null and n.event_type like 'ops.%'
      and not exists(
        ...
        where n.event_type='ops.'||a.domain||'.attention'
          and n.entity_type=a.entity_type and n.entity_id=a.entity_id
      )
    returning 1
  )
