...
-- Production Supabase has pgvector in schema extensions. PGlite used by the
-- migration contract does not. Build the same search API in both environments:
-- vector+lexical on Supabase, lexical-only fallback in PGlite.
do $do$
begin
    ...
    execute $sql$
      ...
      as $fn$
        ...
        chunk_hits as (
          select 'chunk'::text source_type,d.id::text source_id,d.project_id,d.name title,left(dc.content,1800) context,d.category,
            jsonb_build_object('document_id',d.id,'version_id',dv.id,'chunk_no',dc.chunk_no,'page',dc.metadata->'page','section',dc.section_title) source_locator,
            coalesce(ts_rank(to_tsvector('simple',coalesce(dc.content,'')),q.value),0)::real score
          from public.document_chunks dc
          join public.document_versions dv on dv.id=dc.document_version_id
          join public.documents d on d.id=dv.document_id
          cross join q
