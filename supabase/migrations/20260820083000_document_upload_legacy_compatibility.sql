...
-- Certified audit follow-up: production still carries the original document columns
-- alongside the newer R2 upload columns. Keep both representations populated so
-- the atomic upload works on upgraded databases and all readers see one record.
...
revoke all on function public.complete_document_upload(uuid,uuid,uuid,uuid,text,text,text,bigint,text,text,text,text,uuid,timestamptz) from public, anon, authenticated;
grant execute on function public.complete_document_upload(uuid,uuid,uuid,uuid,text,text,text,bigint,text,text,text,text,uuid,timestamptz) to service_role;

insert into public.app_schema_versions(version)
values ('20260820_document_upload_legacy_compatibility')
