create or replace function public.bulk_apply_hr_timesheets_400(
  ...
  p_mode text default 'fill_missing'::text
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  foreach v_employee_id in array p_employee_ids loop
    foreach v_work_date in array p_dates loop
      ...
      if exists (
        select 1 from public.hr_day_statuses d
        where d.workspace_id = p_workspace_id
          and d.employee_id = v_employee_id
          and d.work_date = v_work_date
          and d.status = 'sick'
      ) then
        v_skipped_sick := v_skipped_sick + 1;
        continue;
      end if;
...
comment on function public.bulk_apply_hr_timesheets_400(uuid, uuid, uuid[], date[], uuid, numeric, numeric, text)
is 'Masowa ewidencja czasu: pomija zatwierdzone urlopy/nieobecności oraz niezależne znaczniki Chorobowe.';