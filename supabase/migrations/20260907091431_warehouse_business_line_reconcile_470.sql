...
begin
  ...
  -- Remove only a repeated logical line whose removal brings the sum of lines
  -- materially closer to the invoice header. This preserves legitimate repeated
  -- positions when their multiplicity is required to reconcile the invoice total.
  ...
  -- Canonicalize VAT scale and net unit price. Overlapping chunks sometimes return
  -- VAT as 0.23 vs 23 and a display unit price inconsistent with quantity × net.
  v_next := '[]'::jsonb;
...
drop trigger if exists warehouse_00_sanitize_business_documents_470 on public.document_extractions;
create trigger warehouse_00_sanitize_business_documents_470
before insert or update of payload on public.document_extractions
for each row
when (new.extraction_type='document_context')
execute function private.sanitize_warehouse_extraction_470();

revoke all on function private.sanitize_warehouse_business_document_470(jsonb) from public;
revoke all on function private.sanitize_warehouse_extraction_470() from public;
