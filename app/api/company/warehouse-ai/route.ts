...
type DecisionEvent = {
  ...
  before_candidate_stock_item_id: string | null;
  before_match_confidence: number | null;
  before_reason: string | null;
  after_decision: string;
  after_candidate_stock_item_id: string | null;
...
async function counterpartyIdForReview(workspaceId: string, review: Review) {
  ...
  if (review.supplier_tax_id) {
    const { data } = await db.from("counterparties")
      .select("id")
      .eq("workspace_id", workspaceId)
      .eq("tax_id", review.supplier_tax_id)
      .limit(1)
      .maybeSingle<{ id: string }>();
    if (data?.id) return data.id;
  }

  if (!review.supplier_name) return null;
  const supplierName = review.supplier_name.trim();
  const supplierKey = await normalizeKey(supplierName);
  const { data } = await db.from("counterparties")
    .select("id,name")
    .eq("workspace_id", workspaceId)
    .eq("active", true)
  ...
  const exact = data?.find((row) => row.name?.trim().toLocaleLowerCase("pl") === supplierName.toLocaleLowerCase("pl"));
  if (exact?.id) return exact.id;

  for (const row of data ?? []) {
    if (await normalizeKey(row.name) === supplierKey) return row.id;
  }
  return null;
}
...
async function learnAlias(input: { workspaceId: string; stockItemId: string; line: AiLine; review: Review; userId: string }) {
  ...
  let query = db.from("material_aliases")
  ...
  if (existing) {
    ...
    const { error } = await db.from("material_aliases").insert({
      ...payload,
...
async function recordFeedback(input: { workspaceId: string; line: AiLine; review: Review; candidateId: string; feedback: "accepted" | "rejected"; userId: string }) {
  ...
  const { data: existing } = await db.from("warehouse_ai_feedback")
    ...
    .eq("candidate_stock_item_id", input.candidateId)
    .eq("feedback", input.feedback)
    .maybeSingle<{ id: string; hit_count: number }>();
  if (existing) {
    const { error } = await db.from("warehouse_ai_feedback")
      .update({ hit_count: Number(existing.hit_count ?? 0) + 1, updated_at: new Date().toISOString() })
      .eq("id", existing.id);
    if (error) throw new Error(`Nie udało się zapisać uczenia AI: ${error.message}`);
  } else {
    const { error } = await db.from("warehouse_ai_feedback").insert({
      workspace_id: input.workspaceId,
      normalized_description: normalized,
      supplier_key: supplierKey,
      supplier_sku: input.line.supplier_sku,
      candidate_stock_item_id: input.candidateId,
...
async function recordEvent(workspaceId: string, line: AiLine, afterDecision: string, afterCandidateId: string | null, userId: string) {
  ...
  const { data, error } = await db.from("warehouse_ai_decision_events").insert({
    ...
    ai_line_id: line.id,
    before_decision: line.decision,
    before_candidate_stock_item_id: line.candidate_stock_item_id,
    before_match_confidence: line.match_confidence,
    before_reason: line.decision_reason,
    after_decision: afterDecision,
    after_candidate_stock_item_id: afterCandidateId,
    created_by: userId
...
async function finalizeReview(workspaceId: string, reviewId: string, userId: string) {
  const db = createServiceSupabaseClient();
  const { data, error } = await db.rpc("finalize_warehouse_review_atomic", {
...
async function removeDraftIfStillEditable(workspaceId: string, review: Review) {
  ...
  const db = createServiceSupabaseClient();
  const { data: movement } = await db.from("stock_movements")
    .select("id,status,source_group_key")
    .eq("workspace_id", workspaceId)
    .eq("id", review.draft_movement_id)
    .maybeSingle<{ id: string; status: string; source_group_key: string | null }>();
  if (movement?.status === "draft" && movement.source_group_key === "warehouse-ai-31") {
    const { error: linesError } = await db.from("stock_movement_lines").delete().eq("workspace_id", workspaceId).eq("movement_id", movement.id);
    if (linesError) throw new Error(`Nie udało się cofnąć szkicu ruchu: ${linesError.message}`);
    const { error: movementError } = await db.from("stock_movements").delete().eq("workspace_id", workspaceId).eq("id", movement.id);
    if (movementError) throw new Error(`Nie udało się cofnąć szkicu ruchu: ${movementError.message}`);
    const { error: reviewError } = await db.from("warehouse_document_reviews")
      .update({ draft_movement_id: null, updated_at: new Date().toISOString() })
      .eq("workspace_id", workspaceId)
      .eq("id", review.id);
    if (reviewError) throw new Error(`Nie udało się odpiąć szkicu od dokumentu: ${reviewError.message}`);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    if (body.action === "match") {
      ...
      const { data: stockItem } = await db.from("stock_items")
        .select("id")
        .eq("workspace_id", workspaceId)
        .eq("id", stockItemId)
        .eq("active", true)
    ...
    if (body.action === "create") {
      ...
      const { data: exactName } = await db.from("stock_items")
        .select("id,name")
        .eq("workspace_id", workspaceId)
        .eq("active", true)
        .ilike("name", name)
      ...
      if (line.candidate_stock_item_id) {
        await recordFeedback({ workspaceId, line, review, candidateId: line.candidate_stock_item_id, feedback: "rejected", userId: user.id });
      }
      const { error } = await db.from("warehouse_ai_lines").update({
        ...
        decision_reason: "Utworzono nową kartotekę na podstawie pozycji dokumentu.",
    ...
    if (body.action === "ignore_document") {
      ...
      const { error } = await db.from("warehouse_document_reviews")
        .update({ status: "ignored", draft_movement_id: null, updated_at: new Date().toISOString() })
        .eq("workspace_id", workspaceId)
        .eq("id", reviewId);
      if (error) throw new Error(error.message);
    ...
    if (body.action === "undo") {
      ...
      const { error: eventUpdateError } = await db.from("warehouse_ai_decision_events")
    ...
    if (body.action === "rename_item") {
      ...
      const { data: updated, error } = await db.from("stock_items")
