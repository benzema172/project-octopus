...
type Body = { projectId?: string; action?: "set_requirement" | "generate" | "approve" | "archive"; requirementId?: string; complete?: boolean; documentId?: string | null; outputId?: string };
...
function makePdf(snapshot: Record<string, unknown>, title: string) {
  ...
  const lines = [title, `Inwestycja: ${ascii(project.name)}`, `Inwestor: ${ascii(project.investor)}`, `Lokalizacja: ${ascii(project.location)}`, `Kompletnosc: ${ascii(snapshot.completenessPercent)}% (${ascii(snapshot.complete)}/${ascii(snapshot.required)})`, "", "CHECKLISTA ZAMKNIECIA:"];
...
export async function POST(request: Request) {
  ...
  try {
    const {user,project}=await context(request,body.projectId,"write");
    const db=createServiceSupabaseClient();

    if(body.action==="archive"){
      const {data,error}=await db.rpc("archive_project_atomic",{p_workspace_id:project.workspace_id,p_project_id:project.id,p_actor_id:user.id}).single<{result_id:string;result_status:string}>();
      if(error||!data)throw new Error(error?.message??"Nie zarchiwizowano inwestycji.");
      return NextResponse.json({ok:true,id:data.result_id,status:data.result_status});
    }

    if(["completed","archived"].includes(String(project.status))){
      throw new Error("Zakończona lub zarchiwizowana inwestycja jest tylko do odczytu. Możesz nadal pobierać jej dokumentację i paczki zamknięcia.");
    }
    ...
    if(body.action==="set_requirement"){
      ...
      if(error||!data)throw new Error(error?.message??"Nie zapisano pozycji.");
      return NextResponse.json({ok:true,id:data.result_id,status:data.result_status});
    }
    if(body.action==="generate"){
      ...
      if(error||!data)throw new Error(error?.message??"Nie wygenerowano paczki.");
      return NextResponse.json({ok:true,id:data.result_id,version:data.result_version,status:data.result_status});
    }
    ...
    if(error||!data)throw new Error(error?.message??"Nie zatwierdzono paczki.");
    return NextResponse.json({ok:true,id:data.result_id,status:data.result_status});
