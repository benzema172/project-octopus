...
export default async function CompanyDashboard({ params }: CompanyDashboardProps) {
  ...
  return <main className="co-page co-dashboard-simplified">
    ...
    <section className="co-metric-grid co-metric-grid--decision" aria-label="Najważniejsze informacje">
      <article className="co-metric-card"><span>Aktywne inwestycje</span><strong>{activeProjects}</strong></article>
      <article className="co-metric-card"><span>Do zrobienia</span><strong>{visibleActions.length}</strong></article>
      <article className={`co-metric-card${criticalActions ? " co-metric-card--danger" : ""}`}><span>Krytyczne</span><strong>{criticalActions}</strong></article>
    </section>
