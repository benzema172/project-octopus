      ...
      <section className="pw-finance-card">
        ...
        {financeAllowed ? <>
          <div className="pw-finance-metrics">
            ...
            <div className={forecastMargin != null && forecastMargin >= 0 ? "pw-finance-metric--profit" : undefined}><span className="pw-finance-icon"><BarChart3 size={19} /></span><small>Marża prognozowana</small><strong>{money(forecastMargin)}</strong><p>z ostatniej prognozy</p></div>
          </div>
      ...
      <section className="pw-ops-grid pw-ops-grid--primary">
        <article className="pw-ops-card pw-progress-card">
          ...
          <div className="pw-progress-content">
            ...
            <dl><div><dt>Planowany koniec</dt><dd>{dateLabel(finish)}</dd></div><div><dt>Pozostało</dt><dd>{daysRemaining == null ? "—" : `${daysRemaining} dni`}</dd></div><div><dt>Podstawa</dt><dd>{boqValue > 0 ? "Zatwierdzony BOQ" : "Brak bazowego BOQ"}</dd></div></dl>
          </div>
