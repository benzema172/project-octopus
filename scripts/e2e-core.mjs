...
async function mustInsert(query,label){const result=await query;if(result.error)throw new Error(`${label}: ${result.error.message}`);return result.data;}

try{
 ownerUser=await createUser(ownerEmail);workerUser=await createUser(workerEmail);
 const {data:workspace,error:workspaceError}=await admin.from("workspaces").insert({name:`Octopus E2E ${id}`,owner_id:ownerUser.id}).select("id").single();if(workspaceError||!workspace)throw new Error(workspaceError?.message??"Workspace creation failed");workspaceId=workspace.id;
 await mustInsert(admin.from("workspace_members").insert([{workspace_id:workspaceId,user_id:ownerUser.id,role:"owner"},{workspace_id:workspaceId,user_id:workerUser.id,role:"member"}]),"Membership creation failed");
 const {data:project,error:projectError}=await admin.from("projects").insert({workspace_id:workspaceId,name:`E2E inwestycja ${id}`,status:"active",created_by:ownerUser.id}).select("id").single();if(projectError||!project)throw new Error(projectError?.message??"Project creation failed");
 await mustInsert(admin.from("project_facts").insert({project_id:project.id,fact_type:"project_profile",value_text:"E2E",value_json:{projectName:`E2E inwestycja ${id}`,status:"active",contractValue:"250000"},confidence:1,status:"approved"}),"Project profile creation failed");
 await mustInsert(admin.from("domain_role_grants").insert({workspace_id:workspaceId,user_id:workerUser.id,domain:"investments",access_level:"read",project_id:project.id,granted_by:ownerUser.id}),"Role grant creation failed");
 ...
 await mustInsert(admin.from("warehouses").insert([{id:warehouseA,workspace_id:workspaceId,name:"E2E A"},{id:warehouseB,workspace_id:workspaceId,name:"E2E B"}]),"Warehouses creation failed");
 await mustInsert(admin.from("stock_items").insert({id:itemId,workspace_id:workspaceId,sku:`E2E-${id}`,name:"Rura testowa DN110",unit:"m"}),"Stock item creation failed");
 await mustInsert(admin.from("stock_movements").insert({id:movementId,workspace_id:workspaceId,warehouse_id:warehouseA,movement_type:"PZ",status:"approved",approved_by:ownerUser.id,approved_at:new Date().toISOString()}),"PZ creation failed");
 await mustInsert(admin.from("stock_movement_lines").insert({workspace_id:workspaceId,movement_id:movementId,stock_item_id:itemId,quantity:10}),"PZ line creation failed");
 ...
 try { if(ownerUser?.id) await admin.auth.admin.deleteUser(ownerUser.id); } catch {}
 try { if(workerUser?.id) await admin.auth.admin.deleteUser(workerUser.id); } catch {}
}
