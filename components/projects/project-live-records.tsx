...
export async function ProjectLiveRecords({ projectId, kind }: { projectId: string; kind: Kind }) {
  ...
  return <section className="section-band live-records"><div className="section-heading"><div><p className="eyebrow">Dane na żywo</p><h2>{heading}</h2></div><span>{records.length} rekordów</span></div><div className="live-record-list">{records.map((record) => <article key={record.id}>{["approved", "accepted", "complete", "closed"].includes(record.status) ? <CheckCircle2 size={18} /> : <CircleDashed size={18} />}<div><strong>{record.title}</strong><small>{record.meta}</small></div><span className="status-chip">{record.status}</span>{record.actionable ? <ReviewDecisionButton entityType="estimate_import" entityId={record.id} workspaceId={project.workspace_id} /> : null}</article>)}{records.length === 0 ? <div className="empty-state"><Database size={24} /><strong>Moduł oczekuje na dane</strong><span>{empty}</span><Link href={`/workspace/projects/${projectId}/documentation?upload=1`} className="text-link">Dodaj dokument <ArrowRight size={15} /></Link></div> : null}</div></section>;
}
