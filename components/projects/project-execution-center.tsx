...
import { ArrowRight, Boxes, CalendarRange, FileCheck2, GitCompareArrows, PackageCheck, ShieldCheck, TrendingUp } from "lucide-react";
...
export function ProjectExecutionCenter({ workspaceId, projectId, snapshot }: { workspaceId: string; projectId: string; snapshot: ProjectExecutionSnapshot }) {
  ...
  const stages = [
    ...
    { label: "Łańcuch materiału", value: snapshot.materialEvents, detail: "zdarzenia od wniosku do zużycia", icon: PackageCheck, href: `${base}/warehouse` },
    ...
    <div className="execution-center">
      ...
      <section className="control-dashboard-grid">
        ...
        <article className="module-panel module-panel--ai">
          ...
          <OperationsActionButton projectId={projectId} action="create_forecast" label="Przelicz forecast" />
        </article>
        <article className="module-panel">
          ...
          <OperationsActionButton projectId={projectId} action="initialize_closeout" label="Aktualizuj checklistę" />
        </article>
        <article className="module-panel">
