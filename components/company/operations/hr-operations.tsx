...
export default function HrOperations({ workspaceId,data,canWrite,canApprove,pathname,query }: { workspaceId:string; data:Data; canWrite:boolean; canApprove:boolean; pathname:string; query:string }) {
  ...
  const pendingLeaves=leaves.filter(row=>["pending","submitted","review"].includes(String(row.status))).map(row=>({...row,name:`${names.get(String(row.employee_id))??"Pracownik"} · ${str(row.date_from)}–${str(row.date_to)}`}));
  const pendingTimesheets=timesheets.filter(row=>["draft","submitted","pending"].includes(String(row.status))).map(row=>({...row,name:`${names.get(String(row.employee_id))??"Pracownik"} · ${str(row.work_date)} · ${num(row.hours)} h`}));
