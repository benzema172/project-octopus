...
export default function WarehouseOperations({ workspaceId,data,canWrite,canApprove,pathname,query }: { workspaceId:string; data:Data; canWrite:boolean; canApprove:boolean; pathname:string; query:string }) {
  ...
  const balanceByItem=new Map<string,number>(); balances.forEach(row=>{const id=String(row.stockItemId ?? row.stock_item_id);balanceByItem.set(id,(balanceByItem.get(id)??0)+Number(row.quantity??0));});
  ...
  const metrics=[
    ...
    {label:"Ruchy do akceptacji",value:String(pendingMovements.length),caption:"Oczekują na zatwierdzenie"},
  ...
  return <CompanyModuleShell
    ...
    forms={forms}
    rows={items}
    tableTitle="Kartoteki i stany"
    emptyLabel="Brak kartotek dla bieżącego filtra."
    detailTitle={row=>str(row.name)}
    detailContent={row=>{
      const itemId=String(row.id);
      const itemMovements=movements.filter(item=>String(item.stock_item_id ?? item.stockItemId)===itemId).slice(0,5);
      const itemReservations=reservations.filter(item=>String(item.stock_item_id ?? item.stockItemId)===itemId).slice(0,5);
      const balance=balanceByItem.get(itemId)??0;
      return <>
        <section><h3>Stan kartoteki</h3><p><strong>{num(balance)} {str(row.unit,"")}</strong> na stanie · minimum {num(row.minimum_stock)} {str(row.unit,"")}</p><p>SKU: <strong>{str(row.sku)}</strong> · typ: <strong>{str(row.item_type)}</strong></p></section>
        <section><h3>Ostatnie ruchy</h3>{itemMovements.map(item=><p key={String(item.id)}><strong>{str(item.movement_type)} · {str(item.document_number,"bez numeru")}</strong><br/>{str(item.movement_date,"bez daty")} · {num(item.quantity)} {str(row.unit,"")} · {str(item.status)}</p>)}{!itemMovements.length?<p>Brak ruchów dla tej kartoteki.</p>:null}</section>
        <section><h3>Rezerwacje</h3>{itemReservations.map(item=><p key={String(item.id)}><strong>{num(item.quantity)} {str(row.unit,"")}</strong> · potrzebne {str(item.required_at,"bez terminu")} · {str(item.status)}</p>)}{!itemReservations.length?<p>Brak otwartych rezerwacji dla tej kartoteki.</p>:null}</section>
      </>;
    }}
    columns={[
  ...
  >
    ...
    {reservations.length?<section className="ops-panel ops-panel--compact-list"><h3>Rezerwacje materiałowe</h3>{reservations.slice(0,8).map(row=><p key={String(row.id)}><strong>{num(row.quantity)} · {str(row.status)}</strong> · {str(row.required_at,"bez terminu")}</p>)}</section>:null}
  </CompanyModuleShell>;
}
