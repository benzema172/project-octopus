...
function inRange(date: string, from: unknown, to: unknown) {
  const start = from ? String(from).slice(0, 10) : "0000-01-01";
  const end = to ? String(to).slice(0, 10) : "9999-12-31";
  return start <= date && date <= end;
}

function employedOn(employee: Row, date: string) {
  const hiredAt = employee.hired_at ? String(employee.hired_at).slice(0, 10) : "0000-01-01";
  const terminatedAt = employee.terminated_at ? String(employee.terminated_at).slice(0, 10) : "9999-12-31";
  if (date < hiredAt || date > terminatedAt) return false;
  if (!employee.hired_at && !employee.terminated_at && employee.status && employee.status !== "active") return false;
  return true;
}
...
export function HrDashboardCalendar147({ data }: { data: CalendarData }) {
  ...
  const employeeDay = (employee: Row, date: string): EmployeeDay => {
    ...
    if (!employedOn(employee, date)) {
      return { employee, name: fullName(employee), status: "outside", statusLabel: "Poza zatrudnieniem", location: "—", hours: 0, overtime: 0 };
    }
    ...
    const sheets = data.timesheets.filter((row) => String(row.employee_id) === employeeId && String(row.work_date).slice(0, 10) === date);
    ...
    const sheetProjectIds = sheets.map((row) => {
      if (row.project_id) return String(row.project_id);
      if (row.team_id) return teamProjects.get(String(row.team_id)) ?? "";
      return "";
    }).filter(Boolean);
    const assignmentProjectIds = data.assignments
      .filter((row) => String(row.employee_id) === employeeId && inRange(date, row.date_from, row.date_to))
    ...
    if (leaves.length && (hours > 0 || overtime > 0)) {
      return { employee, name: fullName(employee), status: "conflict", statusLabel: "Urlop + wpis czasu", location, hours, overtime };
    }
    if (leaves.length) {
      const label = leaveLabels[String(leaves[0].leave_type ?? "other")] ?? "Nieobecność";
      return { employee, name: fullName(employee), status: "absence", statusLabel: label, location: label, hours: 0, overtime: 0 };
    }
    if (hours > 0 || overtime > 0) {
      return { employee, name: fullName(employee), status: "work", statusLabel: "Praca", location, hours, overtime };
    }
    return {
      employee,
      name: fullName(employee),
      status: "missing",
      statusLabel: assignmentProjectIds.length ? "Brak wpisu czasu" : "Brak danych",
      location: assignmentProjectIds.length ? location : "Brak przypisania",
      hours: 0,
      overtime: 0
    };
  };
    ...
    <div className={styles.calendarScroll}>
      <div className={styles.calendarGrid} role="grid" aria-label={monthLabel}>
        ...
        {days.map((day) => {
          ...
          const isToday = value === data.referenceDate;
          ...
          const signalTotal = work + absence + missing + conflict;
          return <button
            type="button"
            role="gridcell"
          ...
          >
            <div className={styles.dayTop}>
              ...
              {isToday ? <span className={styles.todayChip}>Dziś</span> : null}
            </div>
            <div className={styles.dayCounters} aria-hidden="true">
              {work ? <span className={styles.counterWork}>{work}</span> : null}
              {absence ? <span className={styles.counterAbsence}>{absence}</span> : null}
              {missing ? <span className={styles.counterMissing}>{missing}</span> : null}
              {conflict ? <span className={styles.counterConflict}>{conflict}</span> : null}
            </div>
            <span className={styles.stateBar} aria-hidden="true">
              {work ? <i className={styles.barWork} style={{ flexGrow: work / Math.max(1, signalTotal) }} /> : null}
              {absence ? <i className={styles.barAbsence} style={{ flexGrow: absence / Math.max(1, signalTotal) }} /> : null}
              {missing ? <i className={styles.barMissing} style={{ flexGrow: missing / Math.max(1, signalTotal) }} /> : null}
              {conflict ? <i className={styles.barConflict} style={{ flexGrow: conflict / Math.max(1, signalTotal) }} /> : null}
            </span>
          </button>;
        })}
      </div>
    ...
    <section className={styles.dayDrawer} key={selectedDate} aria-label={`Status pracowników: ${selectedLabel}`}>
      ...
      <div className={styles.employeeTable}>
        <div className={styles.tableHead}>
          <span><UsersRound size={14} /> Pracownik</span>
          <span><MapPin size={14} /> Gdzie / inwestycja</span>
          <span><Clock3 size={14} /> Godziny</span>
        </div>
        {selectedRows.length ? selectedRows.map((row) => <div className={styles.employeeRow} key={String(row.employee.id)}>
          ...
          <div className={styles.hoursCell}>
            <strong>{formatHours(row.hours + row.overtime)} h</strong>
            {row.overtime > 0 ? <small>w tym {formatHours(row.overtime)} nadg.</small> : null}
          </div>
        </div>) : <div className={styles.empty}>Brak pracowników w ewidencji.</div>}
      </div>
    </section>
  </section>;
}
