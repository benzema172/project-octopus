...
export function HrTeamCostControl156({ workspaceId, data, canWrite, canViewPayroll }: Props) {
  ...
  const visibleProjects = scope === "overplan" ? control.projects.filter((row) => row.overPlan) : control.projects;
  ...
  const submitAssignment = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        ...
        form.reset();
        setFeedback("Pracownik został przypisany. Plan robocizny przeliczy się automatycznie.");
        router.refresh();
      } catch (reason) {
  ...
  return <section className={styles.control} data-hr-labor-cost-control="1">
    <div className={styles.heading}>
      <div>
        <p className={styles.kicker}>Kontrola robocizny</p>
        <h2>Zespoły, godziny i koszt inwestycji</h2>
        <p>Przypisania budują plan. Zatwierdzone karty czasu tworzą koszt rzeczywisty, a wpisy oczekujące są pokazywane osobno.</p>
      </div>
    ...
    <div className={styles.kpis}>
      <button type="button" className={`${styles.kpi} ${scope === "all" ? styles.kpiActive : ""}`} onClick={() => setScope("all")}>
        <Coins size={18} /><span>Koszt robocizny</span><strong>{canViewPayroll ? money(control.actualCost) : "••••"}</strong><small>Zatwierdzone godziny na inwestycjach</small>
      </button>
      ...
      <button type="button" className={`${styles.kpi} ${scope === "unassigned" ? styles.kpiActive : ""}`} onClick={() => setScope(scope === "unassigned" ? "all" : "unassigned")}>
        <BriefcaseBusiness size={18} /><span>Bez inwestycji</span><strong>{canViewPayroll ? money(control.unassignedActualCost) : `${number(control.unassignedApprovedHours)} h`}</strong><small>{number(control.unassignedApprovedHours)} h zatwierdzonego czasu</small>
      </button>
    ...
    {canWrite ? <details className={styles.quickAssign}>
      <summary><Plus size={15} /> Szybkie przypisanie pracownika do inwestycji</summary>
    ...
    {scope !== "unassigned" ? <div className={styles.projectList}>
      {visibleProjects.map((projectCost) => {
        ...
        return <details className={`${styles.projectCard} ${projectCost.overPlan ? styles.projectOver : ""}`} key={projectCost.projectId}>
          <summary>
            <div className={styles.projectIdentity}><UsersRound size={18} /><div><strong>{text(project?.name, "Inwestycja")}</strong><span>{projectCost.people} osób{teamNames ? ` · ${teamNames}` : ""}</span></div></div>
            ...
            <div className={`${styles.delta} ${projectCost.overPlan ? styles.deltaBad : styles.deltaOk}`}>{projectCost.plannedCost > 0 && canViewPayroll ? `${projectCost.overPlan ? "+" : ""}${money(projectCost.actualCost - projectCost.plannedCost)}` : "—"}</div>
          </summary>
          <div className={styles.projectBody}>
            ...
            <div className={styles.tableWrap}><table><thead><tr><th>Pracownik</th><th>Alokacja</th><th>Plan h</th><th>Godziny</th><th>Nadgodziny</th><th>Koszt/h</th><th>Koszt rzeczywisty</th><th>Oczekujące</th><th>Kontrola</th></tr></thead><tbody>
              {projectCost.employeeCosts.map((employeeCost) => {
                ...
                return <tr key={employeeCost.employeeId}>
                  ...
                  <td>{employeeCost.hasAssignment ? `${number(employeeCost.allocationPercent, 0)}%` : <span className={styles.badText}>brak</span>}</td>
                  ...
                  <td><div className={styles.controlFlags}>{!employeeCost.hasAssignment && employeeCost.approvedHours + employeeCost.overtimeHours > 0 ? <span className={styles.flagBad}>Bez przypisania</span> : null}{overHours ? <span className={styles.flagWarn}>Ponad plan h</span> : null}{risk ? <span className={employeeCost.complianceRisk === "expired" ? styles.flagBad : styles.flagWarn}>{risk}</span> : null}{employeeCost.hourlyCost <= 0 && canViewPayroll ? <span className={styles.flagBad}>Brak kosztu/h</span> : null}{employeeCost.hasAssignment && !overHours && !risk && (employeeCost.hourlyCost > 0 || !canViewPayroll) ? <span className={styles.flagOk}>OK</span> : null}</div></td>
                </tr>;
              })}
            </tbody></table></div>
            <div className={styles.projectFooter}>
              <span>Plan: <strong>{number(projectCost.plannedHours)} h</strong>{canViewPayroll ? ` · ${money(projectCost.plannedCost)}` : ""}</span>
              <span>Rzeczywiste: <strong>{number(projectCost.approvedHours + projectCost.overtimeHours)} h</strong>{canViewPayroll ? ` · ${money(projectCost.actualCost)}` : ""}</span>
              <span>Oczekujące: <strong>{number(projectCost.pendingHours)} h</strong>{canViewPayroll ? ` · ${money(projectCost.pendingCost)}` : ""}</span>
            </div>
          </div>
        </details>;
      })}
      {!visibleProjects.length ? <div className={styles.empty}>{scope === "overplan" ? "Brak inwestycji przekraczających plan robocizny." : "Brak przypisań i czasu pracy dla inwestycji w wybranym miesiącu."}</div> : null}
    </div> : null}
    ...
    <p className={styles.sourceNote}>Źródło kosztu: pełny koszt pracodawcy / nominalne godziny. Koszt rzeczywisty powstaje wyłącznie z zatwierdzonej ewidencji czasu, więc może być bezpiecznie używany przez Finanse i KPI inwestycji bez tworzenia drugiego, ręcznego kosztu.</p>
  </section>;
}
