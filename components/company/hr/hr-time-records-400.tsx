...
import { ArrowRight, CalendarDays, Check, ChevronLeft, ChevronRight, Clock3, Download, Filter, Layers3, LoaderCircle, Pencil, RotateCcw, X } from "lucide-react";
...
type DayStatusRow = { id: string; employee_id: string; work_date: string; status: string; source?: string };
type DayStatusResponse = { ok?: boolean; rows?: DayStatusRow[]; error?: string };
type BulkResponse = { ok?: boolean; inserted?: number; updated?: number; skippedExisting?: number; skippedLeave?: number; skippedSick?: number; skippedConflict?: number; affected?: number; error?: string };
type MonthSummary = { work: number; vacation: number; sick: number; absence: number; missing: number; conflict: number };
...
const LEAVE_LABELS: Record<string, string> = { annual: "Urlop wypoczynkowy", on_demand: "Urlop na żądanie", unpaid: "Urlop bezpłatny", sick: "Zwolnienie chorobowe", maternity: "Urlop macierzyński", parental: "Urlop rodzicielski", care: "Opieka", other: "Nieobecność" };
...
function dayStatusKey(employeeId: string, date: string) { return `${employeeId}|${date}`; }
function leaveIsVacation(row?: Row | null) { return Boolean(row && VACATION_TYPES.has(String(row.leave_type ?? ""))); }
function leaveLabel(row?: Row | null) { return row ? LEAVE_LABELS[String(row.leave_type ?? "other")] ?? "Nieobecność" : "Nieobecność"; }
...
function calendarMonthRange(anchorDate: string) {
...
export function HrTimeRecords400({ workspaceId, referenceDate, employees, projects, timesheets, assignments, leaves, canWrite, canViewPayroll, initialEmployeeId = null, onClearEmployeeFocus }: Props) {
  ...
  const dayStatusRange = useMemo(() => mode === "month" ? calendarMonthRange(anchorDate) : range, [mode, anchorDate, range]);
  ...
  const loadDayStatuses = useCallback(async () => {
    const params = new URLSearchParams({ workspaceId, from: dayStatusRange.from, to: dayStatusRange.to });
    setDayStatusError(null);
    try {
      const response = await fetch(`/api/company/hr/day-status?${params.toString()}`, { cache: "no-store" });
      const result = await response.json().catch(() => ({})) as DayStatusResponse;
      if (!response.ok) throw new Error(result.error ?? "Nie udało się pobrać statusów dni.");
      setDayStatuses(result.rows ?? []);
    } catch (reason) {
      setDayStatusError(reason instanceof Error ? reason.message : "Nie udało się pobrać statusów dni.");
    }
  }, [workspaceId, dayStatusRange.from, dayStatusRange.to]);

  useEffect(() => { void loadRange(); }, [loadRange, reloadKey]);
  useEffect(() => { void loadDayStatuses(); }, [loadDayStatuses, reloadKey]);
  ...
  const sickMarker = useCallback((employeeId: string, date: string) => dayStatusIndex.get(dayStatusKey(employeeId, date))?.status === "sick", [dayStatusIndex]);
  const updateDayStatus = useCallback((employeeId: string, date: string, status: "sick" | null) => {
    const key = dayStatusKey(employeeId, date);
    setDayStatuses((current) => {
      const next = current.filter((row) => dayStatusKey(String(row.employee_id), String(row.work_date).slice(0, 10)) !== key);
      if (status === "sick") next.push({ id: `local-${key}`, employee_id: employeeId, work_date: date, status: "sick", source: "calendar" });
      return next;
    });
  }, []);
  ...
  const submitBulk = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setBulkMessage(`Zapisano ${Number(result.affected ?? 0)} wpisów: ${Number(result.inserted ?? 0)} nowych, ${Number(result.updated ?? 0)} poprawionych. Pominięto: ${Number(result.skippedExisting ?? 0)} istniejących, ${Number(result.skippedLeave ?? 0)} urlopów/nieobecności, ${Number(result.skippedSick ?? 0)} dni chorobowego, ${Number(result.skippedConflict ?? 0)} dni z wieloma wpisami.`);
  ...
  const monthDays = useMemo(() => datesBetween(calendarMonthRange(anchorDate).from, calendarMonthRange(anchorDate).to), [anchorDate]);
  ...
  const detailLeave = detailFocus ? approvedLeave(detailFocus.employeeId, detailFocus.workDate) : null;
  const detailSick = detailFocus ? sickMarker(detailFocus.employeeId, detailFocus.workDate) : false;

  return <div className={styles.root} data-hr-time-400="1" data-hr-time-day-status-sync="1">
    <section className={styles.headerCard}>
      <div className={styles.heading}>
        <div><p className={styles.kicker}>Czas pracy 4.0</p><h2>Ewidencja, lokalizacja i historia pracy</h2><p>Przechodź dzień po dniu, kontroluj tydzień, przeglądaj miesiąc albo wyszukuj dowolny historyczny wpis. Urlop i Chorobowe są wspólne dla wszystkich widoków.</p></div>
      ...
      {dayStatusError ? <div className={styles.error}>Statusy Urlop/Chorobowe: {dayStatusError}</div> : null}
    </section>

    {mode === "day" ? <DayView date={anchorDate} employees={visibleEmployees} projects={projects} entriesByEmployeeDate={entriesByEmployeeDate} plannedProjects={plannedProjects} projectNames={projectNames} actualProjects={actualProjects} approvedLeave={approvedLeave} sickMarker={sickMarker} updateDayStatus={updateDayStatus} hasPlanMismatch={hasPlanMismatch} workspaceId={workspaceId} canWrite={canWrite} onChanged={() => setReloadKey((value) => value + 1)} onOpenDetails={(employeeId) => setDetailFocus({ employeeId, workDate: anchorDate })} /> : null}
    {mode === "week" ? <WeekView dates={weekDates} employees={visibleEmployees} projects={projects} entriesByEmployeeDate={entriesByEmployeeDate} plannedProjects={plannedProjects} projectNames={projectNames} approvedLeave={approvedLeave} sickMarker={sickMarker} updateDayStatus={updateDayStatus} hasPlanMismatch={hasPlanMismatch} workspaceId={workspaceId} canWrite={canWrite} onChanged={() => setReloadKey((value) => value + 1)} onOpenDetails={(employeeId, workDate) => setDetailFocus({ employeeId, workDate })} /> : null}
    {mode === "month" ? <MonthView monthKey={monthKey} days={monthDays} employees={visibleEmployees} selectedEmployeeId={employeeFilter} entriesByEmployeeDate={entriesByEmployeeDate} plannedProjects={plannedProjects} projectNames={projectNames} approvedLeave={approvedLeave} sickMarker={sickMarker} hasPlanMismatch={hasPlanMismatch} referenceDate={referenceDate} onOpenDay={(date, employeeId) => { if (employeeId) setDetailFocus({ employeeId, workDate: date }); else { setAnchorDate(date); setMode("day"); } }} /> : null}
    ...
    {bulkOpen && typeof document !== "undefined" ? createPortal(<div className={styles.modalLayer}>
      ...
      <section className={styles.bulkModal} role="dialog" aria-modal="true" aria-labelledby="hr-time-400-bulk-title">
        <header className={styles.detailHeader}><div><span>Edycja zakresowa</span><h3 id="hr-time-400-bulk-title">Przypisz pracowników do inwestycji i godzin</h3><p>Jednym zapisem uzupełnisz serię dni. Urlopy, inne nieobecności i Chorobowe są pomijane, a dni z wieloma istniejącymi wpisami nie są nadpisywane automatycznie.</p></div><button type="button" className={styles.close} disabled={bulkBusy} onClick={() => setBulkOpen(false)}><X size={18} /></button></header>
...
function DayView({ date, employees, projects, entriesByEmployeeDate, plannedProjects, projectNames, actualProjects, approvedLeave, sickMarker, updateDayStatus, hasPlanMismatch, workspaceId, canWrite, onChanged, onOpenDetails }: { date: string; employees: Row[]; projects: Row[]; entriesByEmployeeDate: Map<string, Row[]>; plannedProjects: (employeeId: string, date: string) => string[]; projectNames: (ids: string[]) => string; actualProjects: (rows: Row[]) => string[]; approvedLeave: (employeeId: string, date: string) => Row | null; sickMarker: (employeeId: string, date: string) => boolean; updateDayStatus: (employeeId: string, date: string, status: "sick" | null) => void; hasPlanMismatch: (employeeId: string, date: string, rows: Row[]) => boolean; workspaceId: string; canWrite: boolean; onChanged: () => void; onOpenDetails: (employeeId: string) => void }) {
  return <section className={styles.panel}><header className={styles.panelHeader}><div><p className={styles.kicker}>Dzień</p><h3>Ekipa i lokalizacja</h3></div><span className={styles.help}>Inwestycja, URLOP i CHOROBOWE używają tego samego statusu dnia.</span></header><div className={styles.dayTable}><div className={styles.dayHead}><span>Pracownik</span><span>Plan / status</span><span>Wykonanie i godziny</span><span>Razem</span></div>{employees.map((employee) => { const employeeId = String(employee.id); const dayEntries = entriesByEmployeeDate.get(`${employeeId}|${date}`) ?? []; const planned = plannedProjects(employeeId, date); const actual = actualProjects(dayEntries); const leave = approvedLeave(employeeId, date); const sick = sickMarker(employeeId, date); const mismatch = hasPlanMismatch(employeeId, date, dayEntries); const conflict = (sick && Boolean(leave)) || ((sick || Boolean(leave)) && dayEntries.length > 0); return <div className={styles.dayRow} key={employeeId}><div className={styles.employeeCell}><strong>{employeeName(employee)}</strong><button type="button" onClick={() => onOpenDetails(employeeId)}>Pełne szczegóły <ArrowRight size={12} /></button></div><div className={styles.planCell}>{sick ? <span className={styles.sickChip}>Chorobowe</span> : leave ? <span className={leaveIsVacation(leave) ? styles.vacationChip : styles.leaveChip}>{leaveLabel(leave)}</span> : planned.length ? <><strong>{projectNames(planned)}</strong><small>planowane przypisanie</small></> : <span className={styles.muted}>Brak planu</span>}{conflict ? <span className={styles.dataConflict}>Konflikt danych</span> : mismatch ? <span className={styles.mismatch}>Plan ≠ wykonanie</span> : null}</div><div className={styles.editorCell}><HrTimesheetEntryEditor159 workspaceId={workspaceId} employeeId={employeeId} employeeName={employeeName(employee)} workDate={date} projects={projects} entries={dayEntries} canWrite={canWrite} variant="inline" suggestedProjectId={planned[0] ?? ""} leave={leave} dayStatus={sick ? "sick" : null} enableSickStatus onDayStatusChanged={(status) => updateDayStatus(employeeId, date, status)} onChanged={onChanged} />{actual.length ? <small className={styles.actualLabel}>Wykonanie: {projectNames(actual)}</small> : null}</div><div className={styles.totalCell}><strong>{num(dayEntries.reduce((sum, row) => sum + entryHours(row), 0))} h</strong></div></div>; })}{!employees.length ? <div className={styles.empty}>Brak aktywnych pracowników dla filtra.</div> : null}</div></section>;
}

function WeekView({ dates, employees, projects, entriesByEmployeeDate, plannedProjects, projectNames, approvedLeave, sickMarker, updateDayStatus, hasPlanMismatch, workspaceId, canWrite, onChanged, onOpenDetails }: { dates: string[]; employees: Row[]; projects: Row[]; entriesByEmployeeDate: Map<string, Row[]>; plannedProjects: (employeeId: string, date: string) => string[]; projectNames: (ids: string[]) => string; approvedLeave: (employeeId: string, date: string) => Row | null; sickMarker: (employeeId: string, date: string) => boolean; updateDayStatus: (employeeId: string, date: string, status: "sick" | null) => void; hasPlanMismatch: (employeeId: string, date: string, rows: Row[]) => boolean; workspaceId: string; canWrite: boolean; onChanged: () => void; onOpenDetails: (employeeId: string, date: string) => void }) {
  return <section className={styles.panel}><header className={styles.panelHeader}><div><p className={styles.kicker}>Tydzień</p><h3>Główny ekran korekt</h3></div><span className={styles.help}>Kliknij komórkę, aby zmienić inwestycję, URLOP albo CHOROBOWE również historycznie.</span></header><div className={styles.weekScroll}><table className={styles.weekTable}><thead><tr><th>Pracownik</th>{dates.map((date, index) => <th key={date} className={index > 4 ? styles.weekendHead : undefined}><span>{weekdays[index]}</span><strong>{compactDate(date)}</strong></th>)}<th>Razem</th></tr></thead><tbody>{employees.map((employee) => { const employeeId = String(employee.id); const total = dates.reduce((sum, date) => sum + (entriesByEmployeeDate.get(`${employeeId}|${date}`) ?? []).reduce((value, row) => value + entryHours(row), 0), 0); return <tr key={employeeId}><td><strong>{employeeName(employee)}</strong></td>{dates.map((date, index) => { const dayEntries = entriesByEmployeeDate.get(`${employeeId}|${date}`) ?? []; const planned = plannedProjects(employeeId, date); const leave = approvedLeave(employeeId, date); const sick = sickMarker(employeeId, date); const mismatch = hasPlanMismatch(employeeId, date, dayEntries); const conflict = (sick && Boolean(leave)) || ((sick || Boolean(leave)) && dayEntries.length > 0); const fixedAbsence = Boolean(leave && !leaveIsVacation(leave)); return <td key={date} className={`${styles.weekCell} ${index > 4 ? styles.weekendCell : ""}`}>{fixedAbsence ? <button type="button" className={styles.leaveCell} onClick={() => onOpenDetails(employeeId, date)}><strong>{leaveLabel(leave)}</strong><span>Otwórz szczegóły dnia</span></button> : <><HrTimesheetEntryEditor159 workspaceId={workspaceId} employeeId={employeeId} employeeName={employeeName(employee)} workDate={date} projects={projects} entries={dayEntries} canWrite={canWrite} variant="cell" suggestedProjectId={planned[0] ?? ""} leave={leave} dayStatus={sick ? "sick" : null} enableSickStatus onDayStatusChanged={(status) => updateDayStatus(employeeId, date, status)} onChanged={onChanged} onOpenDetails={() => onOpenDetails(employeeId, date)} />{planned.length && !leave && !sick ? <small className={styles.weekPlan} title={`Plan: ${projectNames(planned)}`}>Plan: {projectNames(planned)}</small> : null}{conflict ? <span className={styles.dataConflictSmall}>konflikt</span> : mismatch ? <span className={styles.mismatchSmall}>≠ plan</span> : null}</>}</td>; })}<td className={styles.weekTotal}><strong>{num(total)} h</strong></td></tr>; })}</tbody></table></div></section>;
}

function MonthView({ monthKey, days, employees, selectedEmployeeId, entriesByEmployeeDate, plannedProjects, projectNames, approvedLeave, sickMarker, hasPlanMismatch, referenceDate, onOpenDay }: { monthKey: string; days: string[]; employees: Row[]; selectedEmployeeId: string; entriesByEmployeeDate: Map<string, Row[]>; plannedProjects: (employeeId: string, date: string) => string[]; projectNames: (ids: string[]) => string; approvedLeave: (employeeId: string, date: string) => Row | null; sickMarker: (employeeId: string, date: string) => boolean; hasPlanMismatch: (employeeId: string, date: string, rows: Row[]) => boolean; referenceDate: string; onOpenDay: (date: string, employeeId?: string) => void }) {
  const selectedEmployee = selectedEmployeeId ? employees.find((row) => String(row.id) === selectedEmployeeId) ?? null : null;
  return <section className={styles.panel}><header className={styles.panelHeader}><div><p className={styles.kicker}>Miesiąc</p><h3>{selectedEmployee ? `Kalendarz — ${employeeName(selectedEmployee)}` : "Kalendarz całej ekipy"}</h3></div><span className={styles.help}>{selectedEmployee ? "Godziny, lokalizacja, urlopy, chorobowe i konflikty danych." : "Kliknij dzień, aby przejść do dziennej korekty całej ekipy."}</span></header><div className={styles.monthGrid}>{weekdays.map((label) => <div className={styles.weekday} key={label}>{label}</div>)}{days.map((date) => { const outside = date.slice(0, 7) !== monthKey; const today = date === referenceDate; if (selectedEmployee) { const id = String(selectedEmployee.id); const rows = entriesByEmployeeDate.get(`${id}|${date}`) ?? []; const leave = approvedLeave(id, date); const sick = sickMarker(id, date); const planned = plannedProjects(id, date); const mismatch = hasPlanMismatch(id, date, rows); const hours = rows.reduce((sum, row) => sum + entryHours(row), 0); const actualIds = Array.from(new Set(rows.map((row) => row.project_id ? String(row.project_id) : "").filter(Boolean))); const conflict = (sick && Boolean(leave)) || ((sick || Boolean(leave)) && rows.length > 0); return <button type="button" key={date} className={`${styles.monthDay} ${outside ? styles.outside : ""} ${today ? styles.today : ""}`} onClick={() => onOpenDay(date, id)}><span className={styles.dayNumber}>{Number(date.slice(8))}</span>{conflict ? <span className={styles.monthConflict}>Konflikt danych</span> : sick ? <span className={styles.monthSick}>Chorobowe</span> : leave ? <span className={leaveIsVacation(leave) ? styles.monthVacation : styles.monthAbsence}>{leaveLabel(leave)}</span> : hours > 0 ? <><strong>{num(hours)} h</strong><small>{actualIds.length ? projectNames(actualIds) : "Koszt ogólny"}</small></> : planned.length ? <><span className={styles.monthMissing}>Brak wpisu</span><small>Plan: {projectNames(planned)}</small></> : <span className={styles.monthEmpty}>—</span>}{mismatch && !conflict ? <span className={styles.mismatchSmall}>≠ plan</span> : null}</button>; }
    const summary = employees.reduce<MonthSummary>((acc, employee) => { const id = String(employee.id); const rows = entriesByEmployeeDate.get(`${id}|${date}`) ?? []; const leave = approvedLeave(id, date); const sick = sickMarker(id, date); const planned = plannedProjects(id, date); const conflict = (sick && Boolean(leave)) || ((sick || Boolean(leave)) && rows.length > 0); if (conflict) acc.conflict += 1; else if (sick) acc.sick += 1; else if (leave) { if (leaveIsVacation(leave)) acc.vacation += 1; else acc.absence += 1; } else if (rows.length) { acc.work += 1; if (hasPlanMismatch(id, date, rows)) acc.conflict += 1; } else if (planned.length) acc.missing += 1; return acc; }, { work: 0, vacation: 0, sick: 0, absence: 0, missing: 0, conflict: 0 }); return <button type="button" key={date} className={`${styles.monthDay} ${outside ? styles.outside : ""} ${today ? styles.today : ""}`} onClick={() => onOpenDay(date)}><span className={styles.dayNumber}>{Number(date.slice(8))}</span><div className={styles.monthSummary}>{summary.work > 0 ? <span className={styles.workCount}>{summary.work} praca</span> : null}{summary.vacation > 0 ? <span className={styles.vacationCount}>{summary.vacation} urlop</span> : null}{summary.sick > 0 ? <span className={styles.sickCount}>{summary.sick} chor.</span> : null}{summary.absence > 0 ? <span className={styles.absenceCount}>{summary.absence} nieob.</span> : null}{summary.missing > 0 ? <span className={styles.missingCount}>{summary.missing} brak</span> : null}{summary.conflict > 0 ? <span className={styles.conflictCount}>{summary.conflict} konflikt</span> : null}</div></button>; })}</div></section>;
}
