...
type DayStatus = "work" | "vacation" | "absence" | "missing" | "outside" | "conflict";
...
export function HrDashboardCalendar159({ workspaceId, canWrite, data, onOpenEmployeeCalendar }: { workspaceId: string; canWrite: boolean; data: CalendarData; onOpenEmployeeCalendar?: (employeeId: string, referenceDate: string) => void }) {
  ...
  }, [approvedLeave, assignmentIds, employeeEntries, projectNames, teamProjects]);
  ...
  const daySummaries = useMemo(() => new Map(days.map((day) => {
    ...
    return [value, { work, vacation, absence, missing, conflict, signalTotal: work + vacation + absence + missing + conflict }];
  ...
  return <section className={styles.calendarPanel} aria-label="Miesięczny kalendarz kadr" data-hr-editable-calendar="1">
    <header className={styles.header}>
      <div className={styles.titleBlock}><span className={styles.icon}><CalendarDays size={18} /></span><div><p className={styles.kicker}>Pulpit kadr</p><h2>Kalendarz miesięczny</h2><p>Kliknij dzień i przypisz pracę albo Urlop. Wybór Urlopu automatycznie tworzy wniosek urlopowy.</p></div></div>
    ...
    <div className={styles.calendarScroll}><div className={styles.calendarGrid} role="grid" aria-label={monthLabel}>
      ...
      {days.map((day) => {
        ...
        const summary = daySummaries.get(value) ?? { work: 0, vacation: 0, absence: 0, missing: 0, conflict: 0, signalTotal: 0 };
        const { work, vacation, absence, missing, conflict, signalTotal } = summary;
        return <button type="button" role="gridcell" aria-selected={isSelected} aria-label={day.toLocaleDateString("pl-PL", { weekday: "long", day: "numeric", month: "long", year: "numeric" })} className={`${styles.day} ${!currentMonth ? styles.outside : ""} ${isWeekend ? styles.weekend : ""} ${isToday ? styles.today : ""} ${isSelected ? styles.selected : ""}`} key={value} onClick={() => setSelectedDate(value)}>
          ...
          <div className={styles.dayCounters} aria-hidden="true">{work ? <span className={styles.counterWork}>{work}</span> : null}{vacation ? <span className={styles.counterVacation}>{vacation}</span> : null}{absence ? <span className={styles.counterAbsence}>{absence}</span> : null}{missing ? <span className={styles.counterMissing}>{missing}</span> : null}{conflict ? <span className={styles.counterConflict}>{conflict}</span> : null}</div>
          <span className={styles.stateBar} aria-hidden="true">{work ? <i className={styles.barWork} style={{ flexGrow: work / Math.max(1, signalTotal) }} /> : null}{vacation ? <i className={styles.barVacation} style={{ flexGrow: vacation / Math.max(1, signalTotal) }} /> : null}{absence ? <i className={styles.barAbsence} style={{ flexGrow: absence / Math.max(1, signalTotal) }} /> : null}{missing ? <i className={styles.barMissing} style={{ flexGrow: missing / Math.max(1, signalTotal) }} /> : null}{conflict ? <i className={styles.barConflict} style={{ flexGrow: conflict / Math.max(1, signalTotal) }} /> : null}</span>
        </button>;
      })}
    </div></div>

    <section className={styles.dayDrawer} key={selectedDate} aria-label={`Status pracowników: ${selectedLabel}`}>
      <header className={styles.drawerHeader}><div><small>Wybrany dzień</small><h3>{selectedLabel}</h3></div><div className={styles.drawerStatusRow}><div className={styles.summaryChips}><span className={styles.summaryWork}>{selectedSummary.work} praca</span><span className={styles.summaryVacation}>{selectedSummary.vacation} urlop</span><span className={styles.summaryAbsence}>{selectedSummary.absence} nieobecność</span><span className={styles.summaryMissing}>{selectedSummary.missing} brak wpisu</span>{selectedSummary.conflict ? <span className={styles.summaryConflict}>{selectedSummary.conflict} konflikt</span> : null}</div><div className={styles.legend} aria-label="Legenda statusów kalendarza"><span><i className={styles.legendWork} /> Praca</span><span><i className={styles.legendVacation} /> Urlop</span><span><i className={styles.legendAbsence} /> Nieobecność</span><span><i className={styles.legendMissing} /> Brak wpisu</span><span><i className={styles.legendConflict} /> Konflikt danych</span></div></div></header>
      <div className={styles.employeeTable}>
        ...
        {selectedRows.length ? selectedRows.map((row) => {
          const employeeId = String(row.employee.id); const entries = employeeEntries(employeeId, selectedDate); const suggestedProjectId = assignmentIds(employeeId, selectedDate)[0] ?? ""; const editable = row.status !== "outside"; const leave = approvedLeave(employeeId, selectedDate);
          return <div className={styles.employeeRow} key={employeeId}>
            ...
            {editable ? <div className={styles.inlineEditorCell}><HrTimesheetEntryEditor159 workspaceId={workspaceId} employeeId={employeeId} employeeName={row.name} workDate={selectedDate} projects={data.projects} entries={entries} canWrite={canWrite} variant="inline" suggestedProjectId={suggestedProjectId} leave={leave} /></div> : <><div className={styles.locationCell}><span>{row.location}</span></div><div className={styles.hoursCell}><strong>{formatHours(row.hours + row.overtime)} h</strong>{row.overtime > 0 ? <small>w tym {formatHours(row.overtime)} nadg.</small> : null}</div></>}
          </div>;
