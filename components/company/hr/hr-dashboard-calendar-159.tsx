...
export function HrDashboardCalendar159({ workspaceId, canWrite, data }: { workspaceId: string; canWrite: boolean; data: CalendarData }) {
      ...
      <div className={styles.employeeTable}>
        ...
        {selectedRows.length ? selectedRows.map((row) => {
          ...
          return <div className={styles.employeeRow} key={employeeId}>
            <div className={styles.employeeCell}><i className={`${styles.statusDot} ${styles[`status_${row.status}`]}`} aria-hidden="true" /><strong>{row.name}</strong></div>
            {editable ? <div style={{ gridColumn: "2 / 4", minWidth: 0 }}><HrTimesheetEntryEditor159 workspaceId={workspaceId} employeeId={employeeId} employeeName={row.name} workDate={selectedDate} projects={data.projects} entries={entries} canWrite={canWrite} variant="inline" suggestedProjectId={suggestedProjectId} /><small className={`${styles.statusBadge} ${styles[`badge_${row.status}`]}`} style={{ display: "inline-flex", marginTop: 5 }}>{row.statusLabel}</small></div> : <><div className={styles.locationCell}><span>{row.location}</span><small className={`${styles.statusBadge} ${styles[`badge_${row.status}`]}`}>{row.statusLabel}</small></div><div className={styles.hoursCell}><strong>{formatHours(row.hours + row.overtime)} h</strong>{row.overtime > 0 ? <small>w tym {formatHours(row.overtime)} nadg.</small> : null}</div></>}
          </div>;
