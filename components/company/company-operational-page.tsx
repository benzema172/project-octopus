...
export async function CompanyOperationalPage({workspaceId,page,query,domain,kind,kicker,title,description,loader}:Props){
  ...
  return <main className="co-page"><header className="co-page-heading"><div><p className="co-kicker">{kicker}</p><h1>{title}</h1><p>{description}</p></div></header><CompanyOperationsLazy workspaceId={workspace.id} kind={kind} data={data} canWrite={canWrite} pathname={pathname} query={query??""}/></main>;
}
