...
export function WarehouseWorkspace300({ workspaceId, data, canWrite, canApprove, query = "" }: Props) {
  ...
  const decisionEvents = (data.warehouseDecisionEvents ?? []) as Row[];
  ...
  const fifoByItem = useMemo(() => {
    ...
    costLayers.forEach((row) => {
      const id = String(row.stock_item_id ?? ""); const quantity = Number(row.remaining_quantity ?? 0); const value = quantity * Number(row.unit_cost ?? 0);
      const current = map.get(id) ?? { quantity: 0, value: 0 }; map.set(id, { quantity: current.quantity + quantity, value: current.value + value });
    });
    return map;
  }, [costLayers]);
  ...
  const run = (endpoint: "ai" | "atomic", action: string, payload: Record<string, unknown>, success: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        const result = await response.json().catch(() => ({})) as { error?: string; eventId?: string; duplicateId?: string; movementId?: string };
        ...
        router.refresh();
      } catch (err) { setError(err instanceof Error ? err.message : "Operacja Magazynu nie powiodła się."); }
    });
  };
  ...
  const doUndo = () => {
    if (!undo) return;
    const eventId = undo.eventId; setUndo(null);
    aiAct("undo", { eventId }, "Cofnięto ostatnią decyzję.");
  };
  const submitSearch = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setTab("stock");
  ...
  return <section className={styles.workspace} data-warehouse-experience="3.1">
    ...
    {tab === "locations" ? <LocationsPanel warehouses={warehouses} locations={locations} balances={balances} aliases={aliases} itemById={itemById} warehouseById={warehouseById} canWrite={canWrite} pending={pending} act={atomicAct} /> : null}
...
function Dashboard({ waiting, lowStock, pendingMovements, catalogItems, movements, quality, fifoByItem, onOpen }: {
  waiting: WarehouseReview300[]; lowStock: Row[]; pendingMovements: Row[]; catalogItems: Row[]; movements: Row[]; quality: Quality;
  fifoByItem: Map<string, { quantity: number; value: number }>; onOpen: (tab: Tab) => void;
}) {
...
function QualityCard({ label, value, note }: { label: string; value: ReactNode; note: string }) { return <div className={styles.qualityCard}><small>{label}</small><strong>{value}</strong><span>{note}</span></div>; }
...
function StockRegistry({ rows, page, query, balanceByItem, reservedByItem, pricesByItem, fifoByItem, counterpartyById, onOpen, onPage }: {
  rows: Row[]; page: PageMeta; query: string; balanceByItem: Map<string, number>; reservedByItem: Map<string, number>; pricesByItem: Map<string, Row[]>;
  fifoByItem: Map<string, { quantity: number; value: number }>; counterpartyById: Map<string, Row>; onOpen: (id: string) => void; onPage: (page: number) => void;
}) {
...
function WaitingRoom({ workspaceId, reviews, currentReview, currentLines, currentPreview, itemById, items, selectedId, onSelect, matchChoice, setMatchChoice, pending, canWrite, act }: {
  workspaceId: string; reviews: WarehouseReview300[]; currentReview: WarehouseReview300 | null; currentLines: WarehouseAiLine300[]; currentPreview: WarehouseDocumentPreview300 | null;
  itemById: Map<string, Row>; items: Row[]; selectedId: string | null; onSelect: (id: string) => void; matchChoice: Record<string, string>; setMatchChoice: (value: Record<string, string>) => void;
  pending: boolean; canWrite: boolean; act: Act;
}) {
...
function MovementsPanel({ rows, lines, warehouses, items, projects, warehouseById, projectById, canWrite, canApprove, pending, act }: {
  rows: Row[]; lines: Row[]; warehouses: Row[]; items: Row[]; projects: Row[]; warehouseById: Map<string, Row>; projectById: Map<string, Row>; canWrite: boolean; canApprove: boolean; pending: boolean; act: Act;
}) {
...
function ManualMovementForm({ warehouses, items, projects, pending, act }: { warehouses: Row[]; items: Row[]; projects: Row[]; pending: boolean; act: Act }) {
  const [open, setOpen] = useState(false); const [type, setType] = useState("PZ"); const [lines, setLines] = useState([{ stockItemId: "", quantity: "", unitCost: "" }]);
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); act("manual_stock_movement", { movementType: type, warehouseId: form.get("warehouseId"), targetWarehouseId: form.get("targetWarehouseId"), projectId: form.get("projectId"), documentNumber: form.get("documentNumber"), movementDate: form.get("movementDate"), lines }, "Utworzono bezpieczny szkic ruchu. Stan nie zmieni się przed zatwierdzeniem."); };
...
function NeedsPanel({ lowStock, reservations, balanceByItem, reservedByItem, pricesByItem, itemById, projectById, warehouseById, counterpartyById, canWrite, pending, act }: {
  lowStock: Row[]; reservations: Row[]; balanceByItem: Map<string, number>; reservedByItem: Map<string, number>; pricesByItem: Map<string, Row[]>; itemById: Map<string, Row>; projectById: Map<string, Row>; warehouseById: Map<string, Row>; counterpartyById: Map<string, Row>; canWrite: boolean; pending: boolean; act: Act;
}) {
...
function PricesPanel({ pricesByItem, items, counterpartyById, purchaseOrders }: { pricesByItem: Map<string, Row[]>; items: Row[]; counterpartyById: Map<string, Row>; purchaseOrders: Row[] }) {
  const alerts = items.flatMap((item) => { const history = pricesByItem.get(String(item.id)) ?? []; if (history.length < 2) return []; const latest = Number(history[0].unit_price_net); const previous = Number(history[1].unit_price_net); const change = previous > 0 ? ((latest - previous) / previous) * 100 : 0; return Math.abs(change) >= 10 ? [{ item, latest: history[0], change }] : []; }).sort((a, b) => Math.abs(b.change) - Math.abs(a.change));
...
function LocationsPanel({ warehouses, locations, balances, aliases, itemById, warehouseById, canWrite, pending, act }: { warehouses: Row[]; locations: Row[]; balances: Row[]; aliases: Row[]; itemById: Map<string, Row>; warehouseById: Map<string, Row>; canWrite: boolean; pending: boolean; act: Act }) {
...
function ItemDrawer({ item, itemId, catalogItems, balances, reservations, aliases, prices, fifo, warehouses, counterparties, locations, locationAssignments, locationById, canWrite, pending, onClose, act }: {
  item?: Row; itemId: string; catalogItems: Row[]; balances: Row[]; reservations: Row[]; aliases: Row[]; prices: Row[]; fifo?: { quantity: number; value: number }; warehouses: Map<string, Row>; counterparties: Map<string, Row>; locations: Row[]; locationAssignments: Row[]; locationById: Map<string, Row>; canWrite: boolean; pending: boolean; onClose: () => void; act: Act;
}) {
  if (!item) return null;
  ...
  const values = prices.map((row) => Number(row.unit_price_net)).filter((value) => Number.isFinite(value) && value > 0); const avg = values.length ? values.reduce((a, b) => a + b, 0) / values.length : 0;
  const similar = catalogItems.filter((row) => String(row.id) !== itemId && similarity(text(row.name,""), text(item.name,"")) >= .55).slice(0, 8);
  const save = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const f = new FormData(event.currentTarget); act("stock_item_update", { stockItemId: itemId, name: f.get("name"), sku: f.get("sku"), barcode: f.get("barcode"), manufacturer: f.get("manufacturer"), model: f.get("model"), itemType: f.get("itemType"), category: f.get("category"), subcategory: f.get("subcategory"), unit: f.get("unit"), minimumStock: f.get("minimum"), optimalStock: f.get("optimal"), warrantyMonths: f.get("warranty"), serialTracking: f.get("serialTracking") === "on", active: f.get("active") === "on" }, "Kartoteka została zaktualizowana."); };
...
function LocationAssignForm({ itemId, locations, pending, act }: { itemId: string; locations: Row[]; pending: boolean; act: Act }) { const submit = (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); const f = new FormData(e.currentTarget); act("warehouse_location_assign", { stockItemId: itemId, locationId: f.get("location"), preferred: f.get("preferred") === "on" }, "Przypisano lokalizację kartoteki."); }; return <form className={styles.inlineAssign} onSubmit={submit}><select name="location" required><option value="">Przypisz do lokalizacji…</option>{locations.map((loc) => <option key={String(loc.id)} value={String(loc.id)}>{text(loc.code)} · {text(loc.name)}</option>)}</select><label><input type="checkbox" name="preferred" /> domyślna</label><button type="submit" disabled={pending}>Przypisz</button></form>; }
function MergeForm({ sourceId, rows, pending, act, onClose }: { sourceId: string; rows: Row[]; pending: boolean; act: Act; onClose: () => void }) { const [target, setTarget] = useState(""); const merge = () => { if (!target || !window.confirm("Scalić kartoteki? Historia ruchów, cen, rezerwacji, faktur i zakupów zostanie przeniesiona do kartoteki docelowej.")) return; act("stock_item_merge", { sourceId, targetId: target }, "Kartoteki scalono bez utraty historii."); onClose(); }; return <div className={styles.mergeForm}><select value={target} onChange={(e) => setTarget(e.target.value)}><option value="">Wybierz kartotekę docelową…</option>{rows.map((row) => <option key={String(row.id)} value={String(row.id)}>{text(row.name)}{row.sku ? ` · ${row.sku}` : ""}</option>)}</select><button type="button" disabled={pending || !target} onClick={merge}>Scal do wybranej</button></div>; }

function similarity(a: string, b: string) { const ta = new Set(normalize(a).split(/[^a-z0-9ąćęłńóśźż]+/).filter((v) => v.length > 1)); const tb = new Set(normalize(b).split(/[^a-z0-9ąćęłńóśźż]+/).filter((v) => v.length > 1)); if (!ta.size || !tb.size) return 0; const common = [...ta].filter((v) => tb.has(v)).length; return common / Math.max(ta.size, tb.size); }
function CompactForm({ title, submit, pending, onSubmit, children, inline = false }: { title: string; submit: string; pending: boolean; onSubmit: (form: FormData) => void; children: ReactNode; inline?: boolean }) { const handle = (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); onSubmit(new FormData(e.currentTarget)); }; return <form className={`${styles.compactForm} ${inline ? styles.compactFormInline : ""}`} onSubmit={handle}><strong>{title}</strong><div>{children}</div><button type="submit" disabled={pending}><Save size={12} /> {submit}</button></form>; }
function SelectField({ name, label, rows, required = false, label: labelFn }: { name: string; label: string; rows: Row[]; required?: boolean; label?: (row: Row) => string }) { return <label>{label}<select name={name} required={required}><option value="">{required ? "Wybierz" : "—"}</option>{rows.map((row) => <option key={String(row.id)} value={String(row.id)}>{labelFn ? labelFn(row) : text(row.name, row.title as string)}</option>)}</select></label>; }
function TextField({ name, label, type = "text", required = false, defaultValue }: { name: string; label: string; type?: string; required?: boolean; defaultValue?: string }) { return <label>{label}<input name={name} type={type} required={required} defaultValue={defaultValue} /></label>; }
