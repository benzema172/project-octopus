...
export function WarehouseUx440({ workspaceId, canWrite, warehouses, initialTab }: Props) {
  ...
  useEffect(() => {
    ...
    const scheduleMovementSuccessDismiss = (scope: HTMLElement) => {
      const banners = Array.from(scope.children).filter(
    ...
    const removeLocationsUi = (scope: HTMLElement) => {
      const navNode = scope.querySelector<HTMLElement>('nav[aria-label="Sekcje Magazynu 3.1"]');
      const buttons = navNode ? Array.from(navNode.querySelectorAll<HTMLButtonElement>("button")) : [];
      ...
      const locationForm = Array.from(scope.querySelectorAll<HTMLElement>("form")).find(
        (form) => normalizedLabel(form.querySelector(":scope > strong")?.textContent) === "Dodaj lokalizację / regał"
      );
    ...
    const sync = () => {
      ...
      removeLocationsUi(scope);
      scheduleMovementSuccessDismiss(scope);
      applyVisibility();
      syncStockSearchHost();
      syncEquipmentHost();
    };
