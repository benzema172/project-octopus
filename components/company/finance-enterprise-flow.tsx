...
function Panel({ id, title, eyebrow, meta, activeId, onToggle, children }: PanelProps) {
  const open = activeId === id;
  return <details className="ops-panel ops-disclosure ops-panel--wide" open={open}>
    <summary className="ops-panel__summary" onClick={(event) => { event.preventDefault(); onToggle(open ? null : id); }}>
      <div><p className="eyebrow">{eyebrow}</p><h2>{title}</h2></div>{meta ? <span>{meta}</span> : null}<ChevronDown size={18} />
    </summary>
    <div className="ops-panel__content">{children}</div>
  </details>;
}
...
export function FinanceEnterpriseFlow({ workspaceId, data, canWrite, canApprove }: Props) {
  ...
  const [activePanel, setActivePanel] = useState<string | null>(() => data.summary.deviationsOpen > 0 ? "deviations" : null);
  const panelState = { activeId: activePanel, onToggle: setActivePanel };
  ...
  return <section className="co-page enterprise-flow" aria-label="Obieg kosztu firmy">
    ...
    <Panel {...panelState} id="documents" title="Dokumenty" eyebrow="1 · Wejście" meta={`${data.inbox.length} ostatnich`}>
    ...
    <Panel {...panelState} id="procurement" title="Kontrola zakupu" eyebrow="2 · WM → PO → PZ → FV" meta={`${data.summary.matchingReview} do sprawdzenia`}>
      <div className="ops-simple-list">
    ...
    <Panel {...panelState} id="allocation" title="Przypisanie kosztu" eyebrow="3 · Inwestycja / firma" meta="NETTO">
    ...
    <Panel {...panelState} id="accounting" title="Księgowość" eyebrow="4 · Dekret i rozliczenie" meta={`${data.summary.accountingProposed} do zatwierdzenia`}>
    ...
    <Panel {...panelState} id="prices" title="Historia cen" eyebrow="Analiza pomocnicza" meta={`${data.priceObservations.length} obserwacji`}>
    ...
    <Panel {...panelState} id="deviations" title="Odstępstwa" eyebrow="Wyjątki wymagające decyzji" meta={`${data.summary.deviationsOpen} otwartych`}>
