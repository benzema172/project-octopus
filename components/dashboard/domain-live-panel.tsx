...
export async function DomainLivePanel({ kind, workspaceId }: { kind: Kind; workspaceId?: string }) {
  ...
  const [grantsResult, integrations, rules, notifications, membersResult, projectsResult] = await Promise.all([
    supabase.from("domain_role_grants").select("id,user_id,domain,access_level,project_id,valid_until").eq("workspace_id", workspace.id).or(`valid_until.is.null,valid_until.gt.${new Date().toISOString()}`).order("created_at", { ascending: false }),
    countWorkspace("integration_connections", workspace.id),
  ...
  const grants = (grantsResult.data ?? []).map((grant) => ({ id: String(grant.id), userId: String(grant.user_id), domain: String(grant.domain), accessLevel: String(grant.access_level), projectId: grant.project_id ? String(grant.project_id) : null }));
  const projects = (projectsResult.data ?? []).map((project) => ({ id: String(project.id), name: String(project.name) }));
  return <section className="section-band"><div className="section-heading"><div><p className="eyebrow">Bezpieczeństwo i automatyzacja</p><h2>Role domenowe, integracje i powiadomienia</h2></div></div><div className="live-domain-grid"><article><ShieldCheck size={18} /><span>Aktywne role</span><strong>{grants.length}</strong><small>HR, Finanse, Inwestycje, Magazyn, Flota</small></article><article><PlugZap size={18} /><span>Integracje</span><strong>{integrations}</strong><small>Tylko stan i konfiguracja bez sekretów</small></article><article><Database size={18} /><span>Reguły alertów</span><strong>{rules}</strong><small>Terminy, ryzyka i wyjątki</small></article><article><AlertTriangle size={18} /><span>Historia alertów</span><strong>{notifications}</strong><small>Ślad decyzji i odczytu</small></article></div><article className="module-panel role-grant-panel"><div className="module-panel__heading"><ShieldCheck size={19} /><div><p className="eyebrow">Zasada najmniejszych uprawnień</p><h2>Nadaj lub odbierz rolę domenową</h2></div></div><RoleGrantForm workspaceId={workspace.id} members={(membersResult.data ?? []).map((member) => ({ userId: String(member.user_id), role: String(member.role) }))} projects={projects} grants={grants} /></article><Link href={`/workspace/companies/${workspace.id}/ai-inbox`} className="text-link">Przejdź do decyzji systemowych <ArrowRight size={15} /></Link></section>;
}
