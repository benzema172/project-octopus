...
function groupByEmployee(rows: HrRow[]) {
  ...
  for (const row of rows) {
    ...
    else map.set(employeeId, [row]);
  }
  return map;
}

function currentEmploymentMap(rows: HrRow[], referenceDate: string) {
  const map = new Map<string, HrRow>();
  for (const row of rows) {
    if (!activeOn(row, referenceDate)) continue;
    const employeeId = String(row.employee_id ?? "");
    if (!employeeId) continue;
    const current = map.get(employeeId);
    if (!current || str(row.valid_from).localeCompare(str(current.valid_from)) > 0) map.set(employeeId, row);
  }
  return map;
}
...
export function buildHrEmployeeIssues(data: HrWorkspaceData, options: { canViewPayroll?: boolean } = {}): HrEmployeeIssueSummary {
  ...
  const employmentByEmployee = currentEmploymentMap(data.employments, referenceDate);
  const documentLinksByEmployee = groupByEmployee(data.employeeDocuments);
  const examsByEmployee = groupByEmployee(data.exams);
  const trainingsByEmployee = groupByEmployee(data.trainings);
  const qualificationsByEmployee = groupByEmployee(data.qualifications);
  const leavesByEmployee = groupByEmployee(data.leaves);
  const timesheetsByEmployee = groupByEmployee(data.timesheets);
  const assignmentsByEmployee = groupByEmployee(data.assignments);
  ...
  for (const employee of data.employees) {
    ...
    const documentLinks = (documentLinksByEmployee.get(employeeId) ?? []).filter((row) => normalize(row.status) !== "archived");
    const hasContract = documentLinks.some((link) => {
      const document = documentById.get(String(link.document_id));
      const value = normalize(`${link.document_type ?? ""} ${document?.name ?? ""}`);
      return value.includes("umowa") || value.includes("contract") || value.includes("employment") || value.includes("zatrudn");
    });
    ...
    const pendingLeaves = employeeLeaves.filter((row) => ["pending", "submitted", "review"].includes(String(row.status))).length;
    ...
    const employeeTimesheets = timesheetsByEmployee.get(employeeId) ?? [];
    const pendingTime = employeeTimesheets.filter((row) => ["draft", "pending", "submitted", "review"].includes(String(row.status))).length;
    ...
    const allocation = (assignmentsByEmployee.get(employeeId) ?? [])
      .filter((row) => activeOn(row, referenceDate, "date_from", "date_to"))
