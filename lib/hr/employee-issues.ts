...
function normalize(value: unknown) {
  return str(value).normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[łŁ]/g, "l").toLowerCase();
}
function employeeName(row: HrRow) {
  return `${str(row.first_name)} ${str(row.last_name)}`.trim() || str(row.employee_number) || "Pracownik";
}
function activeOn(row: HrRow, date: string, fromKey = "valid_from", toKey = "valid_to") {
  const from = str(row[fromKey]) || "0000-01-01";
  const to = str(row[toKey]) || "9999-12-31";
  return from.slice(0, 10) <= date && date <= to.slice(0, 10);
}
...
function currentCompliance(rows: HrRow[], employeeId: string, referenceDate: string) {
  return rows.filter((row) => String(row.employee_id) === employeeId && normalize(row.status) !== "archived")
...
export function buildHrEmployeeIssues(data: HrWorkspaceData, options: { canViewPayroll?: boolean } = {}): HrEmployeeIssueSummary {
  ...
  const push = (issue: Omit<HrEmployeeIssue, "id">) => {
    issues.push({ ...issue, id: `${issue.employeeId}:${issue.kind}:${issues.length}` });
  };
  ...
  for (const employee of data.employees.filter((row) => row.status === "active")) {
    ...
    if (!employment) {
      push({ employeeId, employeeName: name, kind: "employment", severity: "critical", title: "Brak aktywnych warunków zatrudnienia", detail: "Uzupełnij formę zatrudnienia, stanowisko i okres obowiązywania.", targetTab: "employees" });
    }

    const documentLinks = data.employeeDocuments.filter((row) => String(row.employee_id) === employeeId && normalize(row.status) !== "archived");
    const hasContract = documentLinks.some((link) => {
      const document = documentById.get(String(link.document_id));
      const text = normalize(`${link.document_type ?? ""} ${document?.name ?? ""}`);
      return text.includes("umowa") || text.includes("contract") || text.includes("employment") || text.includes("zatrudn");
    });
    ...
    const medical = currentCompliance(data.exams, employeeId, referenceDate)[0];
    if (!medical || !medical.valid_until) {
      ...
      if (days < 0) push({ employeeId, employeeName: name, kind: "medical", severity: "critical", title: "Badanie lekarskie wygasło", detail: `Termin minął ${Math.abs(days)} dni temu.`, targetTab: "compliance", dueDate: due, daysToDue: days });
    ...
    const safety = currentCompliance(data.trainings, employeeId, referenceDate).find((row) => row.valid_until);
    if (!safety) {
      ...
      const days = daysBetween(referenceDate, due);
      if (days < 0) push({ employeeId, employeeName: name, kind: "safety", severity: "critical", title: "Szkolenie BHP wygasło", detail: `Termin minął ${Math.abs(days)} dni temu.`, targetTab: "compliance", dueDate: due, daysToDue: days });
    ...
    const qualifications = currentCompliance(data.qualifications, employeeId, referenceDate);
    const expiredQualifications = qualifications.filter((row) => row.valid_until && str(row.valid_until).slice(0, 10) < referenceDate);
    ...
    if (expiredQualifications.length) push({ employeeId, employeeName: name, kind: "qualification", severity: "critical", title: `${expiredQualifications.length} uprawnienień po terminie`, detail: "Sprawdź SEP/UDT/F-Gazy i pozostałe certyfikaty.", targetTab: "compliance" });
    else if (expiringQualifications.length) push({ employeeId, employeeName: name, kind: "qualification", severity: "warning", title: `${expiringQualifications.length} uprawnienień wygasa ≤30 dni`, detail: "Zaplanuj odnowienie przed wygaśnięciem.", targetTab: "compliance" });
    ...
    const allocation = data.assignments
      .filter((row) => String(row.employee_id) === employeeId && activeOn(row, referenceDate, "date_from", "date_to"))
      .reduce((sum, row) => sum + Math.max(0, Number(row.allocation_percent ?? 0)), 0);
