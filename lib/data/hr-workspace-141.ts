...
function employmentForDate(employments: Row[], employeeId: string, date: string) {
  return employments.find((row) => String(row.employee_id) === employeeId
    && dateOnly(row.valid_from || "0000-01-01") <= date
    && (!row.valid_to || dateOnly(row.valid_to) >= date));
}

function hourlyRate(employment?: Row) {
  ...
  return Number.isFinite(monthly) && Number.isFinite(nominal) && monthly > 0 && nominal > 0 ? monthly / nominal : 0;
}
...
export async function getHrWorkspace141Data(workspaceId: string, options: Options = {}) {
  ...
  // Bazowy loader historycznie pobierał karty czasu bez zamrożonych kosztów. Dociągamy je tutaj,
  // aby koszt rzeczywisty nie zmieniał się po późniejszej zmianie stawki pracownika.
  const snapshots = await db.from("timesheets")
    .select("id,hourly_cost_snapshot,labor_cost_snapshot,cost_snapshot_at")
  ...
  // Nie zerujemy ujemnego salda. Przekroczenie limitu jest informacją kontrolną, a nie wartością do ukrycia.
  ...
  const overusedAlerts = leaveBalances
    .filter((row) => Number(row.overused_days ?? 0) > 0)
    .map((row) => {
      ...
      return {
        ...
        title: `${name} przekroczył limit urlopu o ${Number(row.overused_days)} dni`,
        detail: "Sprawdź zatwierdzone wnioski i roczny wymiar urlopu."
      };
    });
  ...
  if (options.includePayroll) {
    ...
    approvedLaborCost = timesheets
      .filter((row) => row.status === "approved" && dateOnly(row.work_date).startsWith(month))
      .reduce((sum, row) => {
        if (row.labor_cost_snapshot !== null && row.labor_cost_snapshot !== undefined) {
          const snapshot = Number(row.labor_cost_snapshot);
          if (Number.isFinite(snapshot) && snapshot >= 0) return sum + snapshot;
        }
        const hours = Math.max(0, Number(row.hours ?? 0)) + Math.max(0, Number(row.overtime_hours ?? 0));
        const employment = employmentForDate(data.employments as Row[], String(row.employee_id), dateOnly(row.work_date));
  ...
  return {
    ...
    timesheets,
    leaveBalances,
    alerts: [...overusedAlerts, ...(data.alerts as Row[])].slice(0, 30),
    summary: {
      ...data.summary,
