...
export async function getFinanceWorkspaceData(workspaceId: string) {
  ...
  const [counterpartiesResult, invoicesResult, paymentsResult, commitmentsResult, projectsResult] = await Promise.all([
    db.from("counterparties").select("id,name,tax_id,role,active").eq("workspace_id", workspaceId).order("name"),
    db.from("invoices").select("id,counterparty_id,invoice_number,direction,issue_date,due_date,gross_amount,paid_amount,status,created_at").eq("workspace_id", workspaceId).order("created_at", { ascending: false }).limit(100),
...
export async function getWarehouseWorkspaceData(workspaceId: string) {
  const db = createServiceSupabaseClient();
  const [warehousesResult, itemsResult, movementsResult, linesResult, reservationsResult, projectsResult] = await Promise.all([
