...
async function getOperationalDocumentImports(workspaceId: string, mode: "finance" | "warehouse") {
  ...
  const [extractionsResult, documentsResult, invoicesResult, movementsResult] = await Promise.all([
    ...
    db.from("documents").select("id,name,category,project_id,ai_status,review_status,updated_at").eq("workspace_id", workspaceId).is("deleted_at", null).order("updated_at", { ascending: false }).limit(500),
    db.from("invoices").select("id,document_id").eq("workspace_id", workspaceId).not("document_id", "is", null).limit(500),
    db.from("stock_movements").select("id,source_document_id,status").eq("workspace_id", workspaceId).not("source_document_id", "is", null).limit(500)
  ]);
...
export async function getFinanceWorkspaceData(workspaceId: string) {
  ...
  const [counterpartiesResult, invoicesResult, invoiceLinesResult, paymentsResult, commitmentsResult, allocationsResult, projectsResult, aiImports] = await Promise.all([
    db.from("counterparties").select("id,name,tax_id,role,active").eq("workspace_id", workspaceId).order("name").limit(500),
    db.from("invoices").select("id,counterparty_id,document_id,invoice_number,direction,issue_date,due_date,currency,net_amount,tax_amount,gross_amount,paid_amount,status,created_at").eq("workspace_id", workspaceId).order("created_at", { ascending: false }).limit(200),
    db.from("invoice_lines").select("id,invoice_id,line_number,description,quantity,unit,unit_price,net_amount,gross_amount").eq("workspace_id", workspaceId).order("line_number").limit(1000),
    db.from("payments").select("id,invoice_id,payment_date,amount,bank_reference,status").eq("workspace_id", workspaceId).order("payment_date", { ascending: false }).limit(250),
    db.from("commitments").select("id,project_id,description,amount,expected_date,status,created_at").eq("workspace_id", workspaceId).order("created_at", { ascending: false }).limit(250),
    db.from("financial_allocations").select("id,project_id,source_id,source_line_id,boq_item_id,wbs_node_id,amount,status").eq("workspace_id", workspaceId).eq("source_type", "invoice").limit(1000),
...
export async function getHrWorkspaceData(workspaceId: string) {
  ...
  const [employeesResult, employmentsResult, qualificationsResult, examsResult, leavesResult, timesheetsResult, projectsResult] = await Promise.all([
    db.from("employees").select("id,employee_number,first_name,last_name,email,phone,status,hired_at,terminated_at,created_at").eq("workspace_id", workspaceId).order("last_name").limit(500),
    db.from("employments").select("id,employee_id,employment_type,position,valid_from,valid_to,monthly_cost,hourly_cost,currency").eq("workspace_id", workspaceId).order("valid_from", { ascending: false }).limit(1000),
    db.from("qualifications").select("id,employee_id,qualification_type,number,issued_at,valid_until,status").eq("workspace_id", workspaceId).order("valid_until").limit(1000),
    db.from("medical_exams").select("id,employee_id,exam_type,examined_at,valid_until,status").eq("workspace_id", workspaceId).order("valid_until").limit(1000),
    db.from("leave_requests").select("id,employee_id,leave_type,date_from,date_to,days,status").eq("workspace_id", workspaceId).order("date_from", { ascending: false }).limit(500),
    db.from("timesheets").select("id,employee_id,project_id,work_date,hours,overtime_hours,status").eq("workspace_id", workspaceId).order("work_date", { ascending: false }).limit(500),
    db.from("projects").select("id,name").eq("workspace_id", workspaceId).order("name").limit(300)
  ]);
  return {
...
export async function getWarehouseWorkspaceData(workspaceId: string) {
  const db = createServiceSupabaseClient();
  const [warehousesResult, itemsResult, movementsResult, linesResult, reservationsResult, projectsResult, aiImports, balances] = await Promise.all([
    db.from("warehouses").select("id,name,location,warehouse_type,active").eq("workspace_id", workspaceId).order("name").limit(300),
    db.from("stock_items").select("id,sku,name,item_type,unit,minimum_stock,serial_tracking,active").eq("workspace_id", workspaceId).order("name").limit(1000),
    db.from("stock_movements").select("id,project_id,warehouse_id,target_warehouse_id,movement_type,document_number,movement_date,status,created_at").eq("workspace_id", workspaceId).order("movement_date", { ascending: false }).limit(250),
    db.from("stock_movement_lines").select("id,movement_id,stock_item_id,quantity,unit_cost,lot_number").eq("workspace_id", workspaceId).order("id", { ascending: false }).limit(1000),
    db.from("reservations").select("id,project_id,warehouse_id,stock_item_id,quantity,required_at,status").eq("workspace_id", workspaceId).order("required_at").limit(250),
    db.from("projects").select("id,name").eq("workspace_id", workspaceId).order("name").limit(300),
...
export async function getFleetWorkspaceData(workspaceId: string) {
  ...
  const [vehiclesResult, fuelResult, tripsResult, serviceResult, documentsResult, damagesResult, projectsResult, employeesResult] = await Promise.all([
    db.from("vehicles").select("id,registration_number,vin,vehicle_type,make,model,production_year,ownership_type,status,current_mileage,created_at").eq("workspace_id", workspaceId).order("registration_number").limit(500),
    db.from("fuel_entries").select("id,vehicle_id,project_id,fueled_at,liters,gross_amount,mileage,created_at").eq("workspace_id", workspaceId).order("fueled_at", { ascending: false }).limit(500),
    db.from("trips").select("id,vehicle_id,employee_id,project_id,started_at,finished_at,start_location,end_location,distance_km,purpose,created_at").eq("workspace_id", workspaceId).order("started_at", { ascending: false }).limit(500),
    db.from("service_orders").select("id,vehicle_id,service_type,opened_at,closed_at,next_due_date,next_due_mileage,cost,status").eq("workspace_id", workspaceId).order("opened_at", { ascending: false }).limit(250),
    db.from("vehicle_documents").select("id,vehicle_id,document_type,number,valid_from,valid_until,status").eq("workspace_id", workspaceId).order("valid_until").limit(250),
    db.from("damage_cases").select("id,vehicle_id,occurred_at,description,status,cost").eq("workspace_id", workspaceId).order("occurred_at", { ascending: false }).limit(250),
    db.from("projects").select("id,name").eq("workspace_id", workspaceId).order("name").limit(300),
