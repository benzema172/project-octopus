...
export async function getProjectReconciliation(workspaceId: string, projectId: string) {
  ...
  const [graphResult, linksResult, ordersResult, requestsResult, counterpartiesResult, stockItemsResult, boqResult, matchesResult, deviationsResult, pricesResult] = await Promise.all([
    ...
    db.from("purchase_orders").select("id,order_number,status,ordered_at,expected_at,total_amount,currency,source_request_id,counterparty_id,emergency_override,deviation_id").eq("workspace_id", workspaceId).eq("project_id", projectId).order("created_at", { ascending: false }).limit(50),
    db.from("material_requests").select("id,title,status,payload").eq("project_id", projectId).eq("status", "approved").order("created_at", { ascending: false }).limit(100),
