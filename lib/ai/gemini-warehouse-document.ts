...
async function geminiRequestWithBackoff(factory: () => Promise<Response>, attempts = 4) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    const fallback = 1_500 * 2 ** (attempt - 1);
    const waitMs = Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0
      ? Math.min(15_000, retryAfterSeconds * 1_000)
      : Math.min(12_000, fallback);
    await delay(waitMs);
  }
  if (last) return last;
...
async function uploadFile(input: { fileName: string; mimeType: string; bytes: Buffer }) {
  ...
  const start = await geminiRequestWithBackoff(() => fetch("https://generativelanguage.googleapis.com/upload/v1beta/files", {
    ...
    body: JSON.stringify({ file: { displayName: input.fileName } }), signal: AbortSignal.timeout(30_000)
  }));
  ...
  const upload = await geminiRequestWithBackoff(() => fetch(uploadUrl, {
    ...
    body: new Uint8Array(input.bytes), signal: AbortSignal.timeout(90_000)
  }));
...
async function deleteFile(name: string) {
  ...
  await fetch(`https://generativelanguage.googleapis.com/v1beta/${name}`, {
    method: "DELETE", headers: { "x-goog-api-key": apiKey }, signal: AbortSignal.timeout(15_000)
  }).catch(() => undefined);
}

async function analyze(input: { fileName: string; mimeType: string; fileUri?: string; extractedText?: string; projectCatalog?: string[] }) {
  const apiKey = requireServerEnv("GEMINI_API_KEY");
...
Dla każdego businessDocuments podaj rzeczywisty sourcePageStart/sourcePageEnd (strony numerowane od 1), documentType, numer dokumentu/KSeF/PO, daty, dostawcę i nabywcę z NIP, walutę i kwoty oraz KAŻDĄ pozycję dokładnie raz. lineType=material tylko dla fizycznego towaru/materiału/urządzenia/części/narzędzia; service dla robocizny, transportu, najmu i usług; other dla rabatów, korekt i pozycji niejednoznacznych. Nie wymyślaj danych. Jeżeli jedna faktura zajmuje kilka stron, pozostaw ją jednym dokumentem z pełnym zakresem stron. Jeżeli kolejna strona zaczyna inną fakturę, utwórz nowy element businessDocuments.

Jeżeli plik nie jest dokumentem magazynowo-finansowym, businessDocuments ma być puste i ustaw rzeczywistą kategorię. projectHint ma wskazać dokładnie jeden wiersz katalogu albo OGÓLNE. Nie zgaduj.
...
KATALOG INWESTYCJI:\n${projectCatalog}\n\nZwróć wyłącznie JSON zgodny ze schematem.`;
  ...
  try {
    response = await geminiRequestWithBackoff(() => fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-goog-api-key": apiKey },
      body: JSON.stringify({
        contents: [{ role: "user", parts }],
        generationConfig: { responseMimeType: "application/json", responseSchema: RESPONSE_SCHEMA, temperature: 0.05, maxOutputTokens: 32_768 }
      }),
  ...
  if (!response.ok) throw new Error(`Gemini Warehouse odrzucił analizę po ponowieniach: HTTP ${response.status} ${await response.text()}`.slice(0, 700));
...
export async function analyzeWarehouseDocumentWithGemini(input: { fileName: string; mimeType: string; bytes?: Buffer; extractedText?: string; projectCatalog?: string[] }) {
  ...
  if (!useFileApi) return analyze({ fileName: input.fileName, mimeType: input.mimeType, extractedText: input.extractedText, projectCatalog: input.projectCatalog });
  ...
  try {
    ...
    return await analyze({ fileName: input.fileName, mimeType: active.mimeType ?? input.mimeType, fileUri: active.uri, projectCatalog: input.projectCatalog });
  } finally {
