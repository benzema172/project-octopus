...
export const AGENT_TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  { name: "get_employee_availability", description: "Pobierz aktywnych pracowników i zaakceptowane/oczekujące urlopy w podanym okresie.", parameters: objectSchema({ dateFrom: string, dateTo: string }, ["dateFrom", "dateTo"]) },
  ...
  { name: "create_task", description: "Utwórz odwracalne zadanie operacyjne dla inwestycji. To bezpieczna akcja L2.", parameters: objectSchema({ projectId: string, title: string, description: string, priority: string, dueAt: string, confidence: number }, ["projectId", "title"]) },
  { name: "create_material_request", description: "Utwórz szkic wniosku materiałowego. Szkic nie jest wysyłany ani zatwierdzany automatycznie.", parameters: objectSchema({ projectId: string, title: string, manufacturer: string, productName: string, model: string, proposedUse: string, complianceSummary: string, stockItemId: string, confidence: number }, ["projectId", "title"]) },
  { name: "create_purchase_draft", description: "Utwórz WYŁĄCZNIE szkic zamówienia i skieruj go do akceptacji. Nigdy nie oznaczaj go jako zamówionego.", parameters: objectSchema({ projectId: string, counterpartyId: string, description: string, stockItemId: string, quantity: number, unit: string, unitPrice: number, expectedAt: string, confidence: number }, ["projectId", "description", "quantity"]) },
  ...
  { name: "request_approval", description: "Utwórz wpis akceptacji dla istniejącego obiektu wysokiego ryzyka.", parameters: objectSchema({ projectId: string, entityType: string, entityId: string, approvalType: string, note: string }, ["entityType", "entityId", "approvalType"]) }
];

export function geminiAgentTools() {
...
async function approvalFor(ctx: AgentExecutionContext, input: { projectId?: string | null; entityType: string; entityId: string; approvalType: string; note?: string }) {
  ...
  const { data, error } = await db.from("approvals").insert({
    ...
    decision_note: input.note ?? "OctopusAI prosi o zatwierdzenie działania."
...
export async function executeAgentTool(ctx: AgentExecutionContext, name: string, rawArgs: Record<string, unknown>) {
  ...
  try {
    if (name === "search_documents") {
      await requireAccess(ctx, "reports", "read");
      const sources = await searchBrainHybrid({ workspaceId: ctx.workspaceId, query: clean(args.query), projectId: uuidLike(args.projectId), limit: numeric(args.limit) || 16 });
      const visible = [];
      for (const source of sources) {
        const domain: AgentDomain = source.sourceType === "chunk" ? (source.category === "warehouse" || source.category === "invoice" ? "warehouse" : "investments") : "investments";
        if (await ctx.canAccess(domain, "read", source.projectId)) visible.push(source);
      }
    ...
    if (name === "get_project_status" || name === "get_project_budget") {
      ...
      const spent = (allocations ?? []).reduce((sum, row) => sum + Number(row.amount ?? 0), 0);
      const result = name === "get_project_budget" ? { project, allocatedCost: spent, allocations: allocations?.length ?? 0 } : { project, openTasks: tasks ?? [], findings: findings ?? [], allocatedCost: spent };
    ...
    if (name === "get_employee_availability") {
      ...
      const [{ data: employees }, { data: leaves }] = await Promise.all([
        ...
        db.from("leave_requests").select("employee_id,leave_type,date_from,date_to,status").eq("workspace_id", ctx.workspaceId).lte("date_from", dateTo).gte("date_to", dateFrom).in("status", ["approved","pending","in_review"]).limit(500)
      ]);
    ...
    if (name === "get_vehicle_status") {
      ...
      const [{ data: vehicles }, { data: predictions }] = await Promise.all([
        ...
        db.from("fleet_maintenance_predictions").select("vehicle_id,prediction_type,severity,title,recommended_date,status,confidence").eq("workspace_id", ctx.workspaceId).in("status", ["new","open"]).limit(300)
      ]);
      ...
      await logTool(ctx, { name, risk: "read", status: "executed", args, output: { vehicles: filtered.length } });
      return { vehicles: filtered, predictions: predictions ?? [] };
    }

    if (name === "create_task") {
      const projectId = uuidLike(args.projectId);
      if (!projectId) throw new Error("Nieprawidłowa inwestycja.");
      await requireAccess(ctx, "investments", "write", projectId);
      const confidence = numeric(args.confidence) || 0.98;
      if (!canExecuteAutonomously({ policy, risk: "reversible", confidence })) {
        await logTool(ctx, { name, risk: "reversible", projectId, status: "denied", reversible: true, confidence, args });
        return { status: "proposal_only", reason: "Polityka autonomii nie pozwala jeszcze tworzyć zadań automatycznie." };
      }
    ...
    if (name === "create_material_request") {
      ...
      const { data, error } = await db.rpc("save_material_request_v2_atomic", {
        p_workspace_id: ctx.workspaceId, p_project_id: projectId, p_request_id: null, p_source_requirement_id: null,
        p_title: clean(args.title), p_manufacturer: clean(args.manufacturer), p_product_name: clean(args.productName), p_model: clean(args.model),
        p_proposed_use: clean(args.proposedUse), p_compliance_summary: clean(args.complianceSummary), p_stock_item_id: uuidLike(args.stockItemId),
        p_boq_item_id: null, p_wbs_node_id: null, p_request_origin: "planned", p_actor_id: ctx.userId
      });
    ...
    if (name === "create_purchase_draft") {
      ...
      const quantity = Math.max(0, numeric(args.quantity));
      const unitPrice = Math.max(0, numeric(args.unitPrice));
    ...
    if (name === "create_protocol_draft") {
      ...
      const { data, error } = await db.rpc("save_protocol_result_atomic", {
        p_workspace_id: ctx.workspaceId, p_project_id: projectId, p_protocol_id: null, p_protocol_requirement_id: null,
        p_protocol_type: clean(args.protocolType) || "ai_draft", p_title: clean(args.title), p_protocol_date: null, p_performed_at: null,
        p_scope: clean(args.scope), p_location: clean(args.location), p_test_medium: "", p_test_pressure: null, p_pressure_unit: "",
        p_test_duration_minutes: null, p_measurement_device: "", p_result: "", p_remarks: "Szkic OctopusAI — wynik, pomiary i podpisy wymagają formalnego uzupełnienia.",
        p_participants: [], p_evidence: [], p_actor_id: ctx.userId
      });
    ...
    if (name === "request_approval") {
      const projectId = uuidLike(args.projectId);
      const entityId = uuidLike(args.entityId);
      if (!entityId) throw new Error("Nieprawidłowy identyfikator obiektu do akceptacji.");
      await requireAccess(ctx, "investments", "write", projectId);
      const approvalId = await approvalFor(ctx, { projectId, entityType: clean(args.entityType), entityId, approvalType: clean(args.approvalType), note: clean(args.note) });
    ...
    await logTool(ctx, { name, risk: "high", status: "failed", args, error: error instanceof Error ? error.message : String(error) }).catch(() => undefined);
    throw error;
  }
}
